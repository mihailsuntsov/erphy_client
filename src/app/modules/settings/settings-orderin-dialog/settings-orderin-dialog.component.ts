...
@Component({
  selector: 'app-settings-orderin-dialog',
  templateUrl: './settings-orderin-dialog.component.html',
  styleUrls: ['./settings-orderin-dialog.component.css'],
  providers: [LoadSpravService,]
})
export class SettingsOrderinDialogComponent implements OnInit {
 ...
 priceUpDownFieldName:string = translate('modules.field.markup'); // Наименование поля с наценкой-скидкой
  ...
  constructor(private http: HttpClient,
    public SettingsDialog: MatDialogRef<SettingsOrderinDialogComponent>,
  ...
  getSettings(){
    ...
    this.http.get('/api/auth/getSettingsOrderin').subscribe
    (
      data => 
      { 
        result=data as any;
  ...
  getStatusesList(){
    ...
    this.loadSpravService.getStatusList(this.settingsForm.get('companyId').value,35) //35 - id документа из таблицы documents
      .subscribe(
          (data) => 
