...
@Component({
  selector: 'app-settings-orderout-dialog',
  templateUrl: './settings-orderout-dialog.component.html',
  styleUrls: ['./settings-orderout-dialog.component.css'],
  providers: [LoadSpravService,]
})
export class SettingsOrderoutDialogComponent implements OnInit {
 ...
 priceUpDownFieldName:string = translate('modules.field.markup'); // Наименование поля с наценкой-скидкой
  ...
  constructor(private http: HttpClient,
    public SettingsDialog: MatDialogRef<SettingsOrderoutDialogComponent>,
  ...
  getSettings(){
    ...
    this.http.get('/api/auth/getSettingsOrderout').subscribe
    (
      data => 
      { 
        result=data as any;
  ...
  getStatusesList(){
    ...
    this.loadSpravService.getStatusList(this.settingsForm.get('companyId').value,36) //36 - id документа из таблицы documents
      .subscribe(
          (data) => 
