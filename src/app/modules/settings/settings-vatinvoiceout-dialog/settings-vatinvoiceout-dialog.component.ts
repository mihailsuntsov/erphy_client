...
@Component({
  selector: 'app-settings-vatinvoiceout-dialog',
  templateUrl: './settings-vatinvoiceout-dialog.component.html',
  styleUrls: ['./settings-vatinvoiceout-dialog.component.css'],
  providers: [LoadSpravService,]
})
export class SettingsVatinvoiceoutDialogComponent implements OnInit {
  ...
  constructor(private http: HttpClient,
    public SettingsDialog: MatDialogRef<SettingsVatinvoiceoutDialogComponent>,
  ...
  getSettings(){
    ...
    this.http.get('/api/auth/getSettingsVatinvoiceout').subscribe
    (
      data => 
      { 
        result=data as any;
        ...
        if(this.isCompanyInList(+result.companyId)){
          ...
          this.searchCagent2Ctrl.setValue(result.cagent2);
  ...
  checkEmptyCagent2Field(){
    if(this.searchCagent2Ctrl.value.length==0){
      this.settingsForm.get('cagent2Id').setValue(null);
      this.settingsForm.get('cagent2').setValue('');
  }};     
  ...
  onCagent2SearchValueChanges(){
    ...
    .subscribe(data => {
      ...
      if (data == undefined) {
        ...
        console.log(this.filteredCagents2)
  }});}

  onSelectCagent2(id:number,name:string){
    this.settingsForm.get('cagent2Id').setValue(+id);
    this.settingsForm.get('cagent2').setValue(name);
  }
  //------------------------------С Т А Т У С Ы-------------------------------------------------
  getStatusesList(){
    this.receivedStatusesList=null;
    this.loadSpravService.getStatusList(this.settingsForm.get('companyId').value,37) //37 - id документа из таблицы documents
      .subscribe(
          (data) => 
