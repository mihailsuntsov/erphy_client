...
@Component({
  ...
  templateUrl: './settings-writeoff-dialog.component.html',
  styleUrls: ['./settings-writeoff-dialog.component.css'],
...
export class SettingsWriteoffDialogComponent implements OnInit {
 ...
 priceUpDownFieldName:string = translate('modules.field.markup'); // Наименование поля с наценкой-скидкой
  ...
  constructor(private http: HttpClient,
    public SettingsDialog: MatDialogRef<SettingsWriteoffDialogComponent>,
  ...
  getSettings(){
    ...
    this.http.get('/api/auth/getSettingsWriteoff').subscribe
    (
      data => 
      { 
        result=data as any;
  ...
  getStatusesList(){
    ...
    this.loadSpravService.getStatusList(this.settingsForm.get('companyId').value,17) //17 - id документа из таблицы documents
      .subscribe(
          (data) => 
