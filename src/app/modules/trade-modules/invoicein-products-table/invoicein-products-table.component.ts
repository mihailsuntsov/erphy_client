...
@Component({
  selector: 'app-invoicein-products-table',
  templateUrl: './invoicein-products-table.component.html',
  styleUrls: ['./invoicein-products-table.component.css'],
  providers: [ProductCategoriesSelectComponent]
})
export class InvoiceinProductsTableComponent implements OnInit {
  formBaseInformation:any;//форма-обёртка для массива форм invoiceinProductTable (нужна для вывода таблицы)
  ...
  //чекбоксы
  selection = new SelectionModel<InvoiceinProductTable>(true, []);// SelectionModel - специальный класс для удобной работы с чекбоксами
  ...
  @Input() parentDocName:string; // Идентификатор документа, в который вызывается данный компонент. Например, Invoicein и т.д.
  ...
  ngOnInit(): void {
    ...
    this.formSearch = new UntypedFormGroup({
      ...
      reserved: new UntypedFormControl                 ('',[]),                      // зарезервировано в этом отделении
  ...
  showColumns(){
    ...
    this.displayedColumns.push('name','product_count','available','reserved','total','product_price','product_sumprice');
    if(this.nds)
  ...
  masterToggle() {
    ...
    this.resetSelecion() :
    this.formBaseInformation.controls.invoiceinProductTable.value.forEach(row => {
  ...
  createCheckedList(){
    this.checkedList = [];
    for (var i = 0; i < this.formBaseInformation.controls.invoiceinProductTable.value.length; i++) {
      if(this.selection.isSelected(this.formBaseInformation.controls.invoiceinProductTable.value[i])){
        this.checkedList.push(this.formBaseInformation.controls.invoiceinProductTable.value[i].row_id);
      }
  ...
  isAllSelected() {//все выбраны
    ...
    const numRows = this.formBaseInformation.controls.invoiceinProductTable.value.length;
    return  numSelected === numRows;//true если все строки выбраны
  }  
  isThereSelected() {//есть выбранные
    return this.selection.selected.length>0;
  } 
  showCheckbox(row:InvoiceinProductTable):boolean{
    return true;
  }
  // --------------------------------------- *** КОНЕЦ ЧЕКБОКСОВ  *** -------------------------------------
    ...
    try 
    {
      if(this.canAutocompleteQuery && this.searchProductCtrl.value.length>1)
      {
        ...
        return this.http.get(
          '/api/auth/getProductsList?searchString='+this.searchProductCtrl.value+'&companyId='+this.company_id+'&departmentId='+this.department_id+'&document_id=0&priceTypeId=0&showRemovedFromSale=true&showNotPurchased=true&showServices=true'
          );
      }else return [];
    } catch (e) {
      return [];
  ...
  getProductsTable(){
    let productsTable: InvoiceinProductTable[]=[];
    //сбрасываем, иначе при сохранении будут прибавляться дубли и прочие глюки
    const control = <UntypedFormArray>this.formBaseInformation.get('invoiceinProductTable');
    this.gettingTableData=true;
    control.clear();
    this.row_id=0;
    this.http.get('/api/auth/getInvoiceinProductTable?id='+this.parentDocId)
        .subscribe(
            data => { 
  ...
  formingProductRowFromSearchForm() {
    return this._fb.group({
      ...
      reserved: new UntypedFormControl (this.formSearch.get('reserved').value,[]), // сколько зарезервировано
  ...
  deleteProductRow(row: InvoiceinProductTable,index:number) {
    const dialogRef = this.ConfirmDialog.open(ConfirmDialog, {
      width: '400px',
      data:
      { 
...
onChangeProductPrice(row_index:number){
  ...
  getProductTable(){
    return this.formBaseInformation.value.invoiceinProductTable;
  }
...
//*****************************************************************************************************************************************/
  ...
  checkIndivisibleErrorOfProductTable(){
    ...
    this.formBaseInformation.value.invoiceinProductTable.map(t =>{
