...
interface MovingProductTable { //интерфейс для товаров, (т.е. для формы, массив из которых будет содержать форма movingProductTable, входящая в formBaseInformation)
  ...
  reserved: number;               // зарезервировано в складе "from"
...
@Component({
  selector: 'app-moving-products-table',
  templateUrl: './moving-products-table.component.html',
  styleUrls: ['./moving-products-table.component.css'],
  providers: [ProductCategoriesSelectComponent,CommonUtilitesService]
})
export class MovingProductsTableComponent implements OnInit {
  formBaseInformation:any;//форма-обёртка для массива форм movingProductTable (нужна для вывода таблицы)
  ...
  //чекбоксы
  selection = new SelectionModel<MovingProductTable>(true, []);// SelectionModel - специальный класс для удобной работы с чекбоксами
  ...
  @Input() parentDockName:string; // Идентификатор документа, в который вызывается данный компонент. Например, Moving и т.д.
  @Input() company_id:number;
  @Input() department_from_id:number;
  @Input() department_to_id:number;
  ...
  ngOnInit(): void {
    ...
    this.formSearch = new FormGroup({
      ...
      product_count : new FormControl           ('',[Validators.required,Validators.min(0.001),Validators.pattern('^[0-9]{1,6}(?:[.,][0-9]{0,3})?\r?$')]),// количество товара к перемещению
      ...
      reserved: new FormControl                 ('',[]),                      // зарезервировано в складе "from"
  ...
  showColumns(){
    ...
    this.displayedColumns.push('name','product_count','edizm','product_price','available','total','reserved','product_sumprice','product_netcost');
    if(this.nds)
  ...
  masterToggle() {
    ...
    this.resetSelecion() :
    this.formBaseInformation.controls.movingProductTable.value.forEach(row => {
  ...
  createCheckedList(){
    this.checkedList = [];
    for (var i = 0; i < this.formBaseInformation.controls.movingProductTable.value.length; i++) {
      if(this.selection.isSelected(this.formBaseInformation.controls.movingProductTable.value[i])){
        this.checkedList.push(this.formBaseInformation.controls.movingProductTable.value[i].row_id);
      }
  ...
  isAllSelected() {//все выбраны
    ...
    const numRows = this.formBaseInformation.controls.movingProductTable.value.length;
    return  numSelected === numRows;//true если все строки выбраны
  }  
  isThereSelected() {//есть выбранные
    return this.selection.selected.length>0;
  } 
  showCheckbox(row:MovingProductTable):boolean{
    return true;
  }
  // --------------------------------------- *** КОНЕЦ ЧЕКБОКСОВ  *** -------------------------------------
  ...
  getProductsList(){ //заполнение Autocomplete для поля Товар
    try 
    {
      ...
      {
        if(this.department_from_id && this.department_to_id)
        {
          if(this.department_from_id != this.department_to_id)
          {
  ...
  onAutoselectProduct(){
    ...
    this.formSearch.get('product_count').setValue(1);                                               // кол-во
    ...
    this.formSearch.get('reserved').setValue(this.filteredProducts[0].reserved);                    // резерв 
    ...
    this.formSearch.get('is_material').setValue(this.filteredProducts[0].is_material);              // материален ли товар
  ...
  onSelectProduct(product:ProductSearchResponse){
    ...
    this.formSearch.get('reserved').setValue(product.reserved);                    // резерв 
    ...
    this.formSearch.get('is_material').setValue(product.is_material);              // материален ли товар
    ...
    this.formSearch.get('available').setValue(product.total-product.reserved);     // доступное кол-во
    this.canAutocompleteQuery=false;
    this.afterSelectProduct();
  }
  ...
  afterSelectProduct(){
    ...
    if(this.autoAdd){
      ...
      this.changeProductsTableLength.emit();//для того, чтобы заблокировать поля Предприятие, Отделения 
  ...
  openDialogProductReserves(productId: number) { //открывает диалог отчета резервов
    const dialogReserves = this.ProductReservesDialogComponent.open(ProductReservesDialogComponent, {
      ...
      { 
        ...
        productId: productId,
        departmentId:this.department_from_id,
      },
    });
  ...
  getProductsTable(){
    let productsTable: MovingProductTable[]=[];
    //сбрасываем, иначе при сохранении будут прибавляться дубли и прочие глюки
    const control = <FormArray>this.formBaseInformation.get('movingProductTable');
    this.gettingTableData=true;
    control.clear();
    this.row_id=0;
    this.http.get('/api/auth/getMovingProductTable?id='+this.parentDockId)
        .subscribe(
            data => { 
  ...
  formingProductRowFromApiResponse(row: MovingProductTable) {
    return this._fb.group({
      ...
      reserved: new FormControl (+row.reserved,[]),
      product_sumprice: new FormControl ((+row.product_count*(+row.product_price)).toFixed(2),[]),
      product_count:  new FormControl (row.product_count,[Validators.required,Validators.min(0.001), Validators.pattern('^[0-9]{1,7}(?:[.,][0-9]{0,3})?\r?$')]),
  ...
  formingProductRowFromSearchForm() {
    return this._fb.group({
      ...
      product_count:  new FormControl (this.formSearch.get('product_count').value,[Validators.required,Validators.min(0.001), Validators.pattern('^[0-9]{1,7}(?:[.,][0-9]{0,3})?\r?$')]),
      total: new FormControl (+this.formSearch.get('total').value,[]),
      reserved: new FormControl (+this.formSearch.get('reserved').value,[]),
  ...
  deleteProductRow(row: MovingProductTable,index:number) {
    const dialogRef = this.ConfirmDialog.open(ConfirmDialog, {
      width: '400px',
      data:
      { 
...
    this.http.get('/api/auth/getShortInfoAboutProduct?department_id='+this.department_from_id+'&product_id='+this.formSearch.get('product_id').value)
      .subscribe(
          data => { 
  ...
  tableRecount(){
    if(this.formBaseInformation!=undefined){//метод может вызываться из ngOnChanges, а т.к. он стартует до ngOnInit, то formBaseInformation может еще не быть
      ...
      //перерасчет себестоимости в таблице товаров
      ...
      console.log('KZ='+KZ);
      this.formBaseInformation.value.movingProductTable.map(i => {
          if(+this.overhead_netcost_method>0){
            //если расходы распределяются по себестоимости
              i['product_netcost']=+((+i['product_price']*KZ)+(+i['product_price'])).toFixed(2);
            }else{ 
            //если расходы НЕ распределяются по себестоимости 
              i['product_netcost']=+(+i['product_price']).toFixed(2);
          } 
      });
...
//при изменении поля Количество в таблице товаров
  onChangeProductCount(row_index:number){
    ...
    this.tableRecount();                                      // пересчёт Суммы оплаты за товар
  ...
  onChangeOverhead(){
    setTimeout(() => { this.tableRecount()}, 1);// пересчёт Суммы оплаты за товар
  }
  ...
  getProductTable(){
    return this.formBaseInformation.value.movingProductTable;
  }
...
//*****************************************************************************************************************************************/
  ...
  checkIndivisibleErrorOfProductTable(){
    ...
    this.formBaseInformation.value.movingProductTable.map(t =>{
...
//*****************************************************************************************************************************************/
  ...
  getProductsInfoListByIds(selection:string){
    const body =  {
      ...
      departmentId:this.department_from_id,   // id отделения
  ...
  formingProductRowFromProductsList(row: ProductSearchResponse) {
    return this._fb.group({
      ...
      edizm: new FormControl (row.edizm,[]),
      total: new FormControl (+row.total,[]),
      product_count:  new FormControl (1,[Validators.required,Validators.min(0.001), Validators.pattern('^[0-9]{1,7}(?:[.,][0-9]{0,3})?\r?$')]),
