...
export class MovingProductsTableComponent implements OnInit {
  ...
  @Input() parentDocName:string; // Идентификатор документа, в который вызывается данный компонент. Например, Moving и т.д.
  ...
  getProductsTable(){
    ...
    control.clear();
    this.row_id=0;
    this.http.get('/api/auth/getMovingProductTable?id='+this.parentDocId)
        .subscribe(
            data => { 
