...
@Component({
  ...
  templateUrl: './posting-products-table.component.html',
  styleUrls: ['./posting-products-table.component.css'],
  providers: [CommonUtilitesService]
})
export class PostingProductsTableComponent implements OnInit {
  formBaseInformation:any;//форма-обёртка для массива форм postingProductTable (нужна для вывода таблицы)
  ...
  //чекбоксы
  selection = new SelectionModel<PostingProductTable>(true, []);// SelectionModel - специальный класс для удобной работы с чекбоксами
  ...
  @Input() parentDocName:string; // Идентификатор документа, в который вызывается данный компонент. Например, Posting и т.д.
  ...
  masterToggle() {
    ...
    this.resetSelecion() :
    this.formBaseInformation.controls.postingProductTable.value.forEach(row => {
  ...
  createCheckedList(){
    this.checkedList = [];
    for (var i = 0; i < this.formBaseInformation.controls.postingProductTable.value.length; i++) {
      if(this.selection.isSelected(this.formBaseInformation.controls.postingProductTable.value[i])){
        this.checkedList.push(this.formBaseInformation.controls.postingProductTable.value[i].row_id);
      }
  ...
  isAllSelected() {//все выбраны
    ...
    const numRows = this.formBaseInformation.controls.postingProductTable.value.length;
    return  numSelected === numRows;//true если все строки выбраны
  }  
  isThereSelected() {//есть выбранные
    return this.selection.selected.length>0;
  } 
  showCheckbox(row:PostingProductTable):boolean{
    return true;
  }
  // --------------------------------------- *** КОНЕЦ ЧЕКБОКСОВ  *** -------------------------------------
    ...
    try 
    {
      if(this.canAutocompleteQuery && this.searchProductCtrl.value.length>1)
      {
        if(this.department_id){
          ...
          '/api/auth/getProductsList?searchString='+this.searchProductCtrl.value+'&companyId='+this.company_id+'&departmentId='+this.department_id+'&document_id=0&priceTypeId='+(+this.priceTypeId)+'&showRemovedFromSale=true&showNotPurchased=true&showServices=false'
          );
        } else {
  ...
  getProductsTable(){
    let productsTable: PostingProductTable[]=[];
    //сбрасываем, иначе при сохранении будут прибавляться дубли и прочие глюки
    const control = <UntypedFormArray>this.formBaseInformation.get('postingProductTable');
    this.gettingTableData=true;
    control.clear();
    this.row_id=0;
    this.http.get('/api/auth/getPostingProductTable?id='+this.parentDocId)
        .subscribe(
            data => { 
  ...
  deleteProductRow(row: PostingProductTable,index:number) {
    const dialogRef = this.ConfirmDialog.open(ConfirmDialog, {
      width: '400px',
      data:
      { 
...
//------------------------------------------------- ON CHANGE...
  ...
  getProductTable(){
    return this.formBaseInformation.value.postingProductTable;
  }
...
//*****************************************************************************************************************************************/
  ...
  checkIndivisibleErrorOfProductTable(){
    ...
    this.formBaseInformation.value.postingProductTable.map(t =>{
