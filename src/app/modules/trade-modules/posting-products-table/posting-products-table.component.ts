...
export class PostingProductsTableComponent implements OnInit {
  ...
  @Input() parentDockName:string; // Идентификатор документа, в который вызывается данный компонент. Например, Posting и т.д.
  ...
  getProductsTable(){
    ...
    control.clear();
    this.row_id=0;
    this.http.get('/api/auth/getPostingProductTable?id='+this.parentDockId)
        .subscribe(
            data => { 
