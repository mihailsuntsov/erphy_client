...
export class ReturnsupProductsTableComponent implements OnInit {
  ...
  @Input() parentDocName:string; // Идентификатор документа, в который вызывается данный компонент. Например, Returnsup и т.д.
      ...
      if(this.canAutocompleteQuery && this.searchProductCtrl.value.length>1)
      {
        if(this.department_id){
          ...
          return this.http.get(
            '/api/auth/getProductsList?searchString='+this.searchProductCtrl.value+'&companyId='+this.company_id+'&departmentId='+this.department_id+'&document_id=0&priceTypeId='+(+this.priceTypeId)+'&showRemovedFromSale=true&showNotPurchased=true&showServices=true'
          );
        } else {
  ...
  getProductsTable(){
    ...
    control.clear();
    this.row_id=0;
    this.http.get('/api/auth/getReturnsupProductTable?id='+this.parentDocId)
        .subscribe(
            data => { 
