...
import { QueryFormService } from './get-accounts-table.service';
...
export class PaymentAccountComponent implements OnInit {
  ...
  receivedPaymentAccountList: idAndName [] = [];//массив для получения списка расходов
  ...
  constructor(private queryFormService:   QueryFormService,
    ...
    public dialogRef1: MatDialogRef<PaymentAccountComponent>,
    ...
    ngOnInit() {
      ...
      if(Cookie.get('accounts_companyId')=='undefined' || Cookie.get('accounts_companyId')==null)     
        Cookie.set('accounts_companyId',this.sendingQueryForm.companyId); else this.sendingQueryForm.companyId=(Cookie.get('accounts_companyId')=="0"?"0":+Cookie.get('accounts_companyId'));
      if(Cookie.get('accounts_sortAsc')=='undefined' || Cookie.get('accounts_sortAsc')==null)       
        Cookie.set('accounts_sortAsc',this.sendingQueryForm.sortAsc); else this.sendingQueryForm.sortAsc=Cookie.get('accounts_sortAsc');
      if(Cookie.get('accounts_sortColumn')=='undefined' || Cookie.get('accounts_sortColumn')==null)    
        Cookie.set('accounts_sortColumn',this.sendingQueryForm.sortColumn); else this.sendingQueryForm.sortColumn=Cookie.get('accounts_sortColumn');
      if(Cookie.get('accounts_offset')=='undefined' || Cookie.get('accounts_offset')==null)        
        Cookie.set('accounts_offset',this.sendingQueryForm.offset); else this.sendingQueryForm.offset=Cookie.get('accounts_offset');
      if(Cookie.get('accounts_result')=='undefined' || Cookie.get('accounts_result')==null)        
        Cookie.set('accounts_result',this.sendingQueryForm.result); else this.sendingQueryForm.result=Cookie.get('accounts_result');
  ...
  getCRUD_rights(){
    ...
    this.allowToDeleteAllCompanies = this.permissionsSet.some(         function(e){return(e==656)});
    this.allowToDeleteMyCompany = this.permissionsSet.some(            function(e){return(e==657)});
...
// -------------------------------------- *** КОНЕЦ ПРАВ *** ------------------------------------
  ...
  setNumOfPages(){
    ...
    this.createCheckedList();
    this.sendingQueryForm.offset=0;
    Cookie.set('accounts_result',this.sendingQueryForm.result);
    this.getData();
  }
  ...
  setPage(value:any) // set pagination
  {
    ...
    Cookie.set('accounts_offset',value);
    this.getData();
  }
  ...
  setSort(valueSortColumn:any) // set sorting column
  {
      ...
      Cookie.set('accounts_sortAsc',this.sendingQueryForm.sortAsc);
      } else {
          this.sendingQueryForm.sortColumn=valueSortColumn;
          this.sendingQueryForm.sortAsc="asc";
          Cookie.set('accounts_sortAsc',"asc");
          Cookie.set('accounts_sortColumn',valueSortColumn);
      }
      this.getData();
  }
  onCompanySelection(){
    Cookie.set('accounts_companyId',this.sendingQueryForm.companyId);
    this.resetOptions();
    this.getCRUD_rights();
  }
  ...
  getPaymentAccountNameById(id:string):string{
    ...
    if(this.receivedPaymentAccountList){
      this.receivedPaymentAccountList.forEach(a=>{
  ...
  undeleteDocs(){
    ...
    this.clearCheckboxSelection();
    return this.http.post('/api/auth/undeletePaymentAccount', body) 
    .subscribe(
        (data) => {   
  ...
  deleteDocs(){
    const body = {"checked": this.checkedList.join()}; //join переводит из массива в строку
    this.clearCheckboxSelection();
    return this.http.post('/api/auth/deletePaymentAccount', body) 
      .subscribe(
        (data) => {  
          let result=data as any;
          switch(result){ //+++
  ...
  setDefaultCompany(){
    if(Cookie.get('accounts_companyId')=='0'||!this.companyIdInList(Cookie.get('accounts_companyId'))){
