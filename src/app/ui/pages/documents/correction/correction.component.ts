...
import { QueryFormService } from './get-correction-table.service';
...
export class CorrectionComponent implements OnInit {
 ...
 visBtnDelete = false;
  ...
  constructor(private queryFormService:   QueryFormService,
    ...
    private settingsCorrectionDialogComponent: MatDialog,
    public deleteDialog: MatDialog,
    public dialogRef1: MatDialogRef<CorrectionComponent>,
    ...
    ngOnInit() {
      ...
      if(Cookie.get('correction_companyId')=='undefined' || Cookie.get('correction_companyId')==null)     
        Cookie.set('correction_companyId',this.sendingQueryForm.companyId); else this.sendingQueryForm.companyId=(Cookie.get('correction_companyId')=="0"?"0":+Cookie.get('correction_companyId'));
      if(Cookie.get('correction_sortAsc')=='undefined' || Cookie.get('correction_sortAsc')==null)       
        Cookie.set('correction_sortAsc',this.sendingQueryForm.sortAsc); else this.sendingQueryForm.sortAsc=Cookie.get('correction_sortAsc');
      if(Cookie.get('correction_sortColumn')=='undefined' || Cookie.get('correction_sortColumn')==null)    
        Cookie.set('correction_sortColumn',this.sendingQueryForm.sortColumn); else this.sendingQueryForm.sortColumn=Cookie.get('correction_sortColumn');
      if(Cookie.get('correction_offset')=='undefined' || Cookie.get('correction_offset')==null)        
        Cookie.set('correction_offset',this.sendingQueryForm.offset); else this.sendingQueryForm.offset=Cookie.get('correction_offset');
      if(Cookie.get('correction_result')=='undefined' || Cookie.get('correction_result')==null)        
        Cookie.set('correction_result',this.sendingQueryForm.result); else this.sendingQueryForm.result=Cookie.get('correction_result');
  ...
  getCRUD_rights(permissionsSet:any[]){
    this.allowToCreateAllCompanies = permissionsSet.some(         function(e){return(e==540)});
    this.allowToCreateMyCompany = permissionsSet.some(            function(e){return(e==541)});
    this.allowToDeleteAllCompanies = permissionsSet.some(         function(e){return(e==542)});
    this.allowToDeleteMyCompany = permissionsSet.some(            function(e){return(e==543)});
    this.allowToViewAllCompanies = permissionsSet.some(           function(e){return(e==544)});
    this.allowToViewMyCompany = permissionsSet.some(              function(e){return(e==545)});
    this.allowToUpdateAllCompanies = permissionsSet.some(         function(e){return(e==546)});
    this.allowToUpdateMyCompany = permissionsSet.some(            function(e){return(e==547)});
    this.getData();
  }
...
// -------------------------------------- *** КОНЕЦ ПРАВ *** ------------------------------------
  ...
  getPagesList(){
    this.queryFormService.getPagesList(this.sendingQueryForm)
            .subscribe(
                ...
                error => {this.gettingTableData=false;this.MessageDialog.open(MessageDialog,{width:'400px',data:{head:translate('menu.msg.error'),message:error.error}})} //+++
            ); 
  }
  ...
  setNumOfPages(){
    ...
    this.createCheckedList();
    this.sendingQueryForm.offset=0;
    Cookie.set('correction_result',this.sendingQueryForm.result);
    this.getData();
  }
  ...
  setPage(value:any) // set pagination
  {
    ...
    Cookie.set('correction_offset',value);
    this.getData();
  }
  ...
  setSort(valueSortColumn:any) // set sorting column
  {
      ...
      Cookie.set('correction_sortAsc',this.sendingQueryForm.sortAsc);
      } else {
          this.sendingQueryForm.sortColumn=valueSortColumn;
          this.sendingQueryForm.sortAsc="asc";
          Cookie.set('correction_sortAsc',"asc");
          Cookie.set('correction_sortColumn',valueSortColumn);
      }
      this.getData();
  }
  onCompanySelection(){
    Cookie.set('correction_companyId',this.sendingQueryForm.companyId);
    this.resetOptions();
    this.getData();
  }
  clickBtnDelete(): void {
  ...
  undeleteDocs(){
    ...
    this.clearCheckboxSelection();
    return this.http.post('/api/auth/undeleteCorrection', body) 
    .subscribe(
      (data) => {  
  ...
  deleteDocs(){
    const body = {"checked": this.checkedList.join()}; //join переводит из массива в строку
    this.clearCheckboxSelection();
    return this.http.post('/api/auth/deleteCorrection', body) 
    .subscribe((data) => {          
      let result=data as any;
  ...
  setDefaultCompany(){
    if(Cookie.get('correction_companyId')=='0'||!this.companyIdInList(Cookie.get('correction_companyId'))){
    ...
    saveSettingsCorrection(){
      ...
      .subscribe(
          (data) => {   
            ...
            error => {this.MessageDialog.open(MessageDialog,{width:'400px',data:{head:translate('menu.msg.error'),message:error.error}})} //+++
        );
    }
  //***********************************************  Ф И Л Ь Т Р   О П Ц И Й   *******************************************/
