...
import { QueryFormService } from './get-currencies-table.service';
...
export class CurrenciesComponent implements OnInit {
  ...
  receivedCurrenciesList: idAndName [] = [];//массив для получения списка расходов
  ...
  constructor(private queryFormService:   QueryFormService,
    ...
    public dialogRef1: MatDialogRef<CurrenciesComponent>,
    ...
    ngOnInit() {
      ...
      this.sendingQueryForm.sortColumn='name_full';
      this.sendingQueryForm.offset='0';
      this.sendingQueryForm.result='10';
      this.sendingQueryForm.searchCategoryString="";
      this.sendingQueryForm.filterOptionsIds = [];

      if(Cookie.get('currencies_companyId')=='undefined' || Cookie.get('currencies_companyId')==null)     
        Cookie.set('currencies_companyId',this.sendingQueryForm.companyId); else this.sendingQueryForm.companyId=(Cookie.get('currencies_companyId')=="0"?"0":+Cookie.get('currencies_companyId'));
      if(Cookie.get('currencies_sortAsc')=='undefined' || Cookie.get('currencies_sortAsc')==null)       
        Cookie.set('currencies_sortAsc',this.sendingQueryForm.sortAsc); else this.sendingQueryForm.sortAsc=Cookie.get('currencies_sortAsc');
      if(Cookie.get('currencies_sortColumn')=='undefined' || Cookie.get('currencies_sortColumn')==null)    
        Cookie.set('currencies_sortColumn',this.sendingQueryForm.sortColumn); else this.sendingQueryForm.sortColumn=Cookie.get('currencies_sortColumn');
      if(Cookie.get('currencies_offset')=='undefined' || Cookie.get('currencies_offset')==null)        
        Cookie.set('currencies_offset',this.sendingQueryForm.offset); else this.sendingQueryForm.offset=Cookie.get('currencies_offset');
      if(Cookie.get('currencies_result')=='undefined' || Cookie.get('currencies_result')==null)        
        Cookie.set('currencies_result',this.sendingQueryForm.result); else this.sendingQueryForm.result=Cookie.get('currencies_result');
  ...
  getCRUD_rights(){
    ...
    this.allowToDeleteAllCompanies = this.permissionsSet.some(         function(e){return(e==647)});
    this.allowToDeleteMyCompany = this.permissionsSet.some(            function(e){return(e==648)});
...
// -------------------------------------- *** КОНЕЦ ПРАВ *** ------------------------------------
  ...
  getTableHeaderTitles(){
    ...
    this.displayedColumns.push('name_full');
    this.displayedColumns.push('name_short');
  ...
  setNumOfPages(){
    ...
    this.createCheckedList();
    this.sendingQueryForm.offset=0;
    Cookie.set('currencies_result',this.sendingQueryForm.result);
    this.getData();
  }
  ...
  setPage(value:any) // set pagination
  {
    ...
    Cookie.set('currencies_offset',value);
    this.getData();
  }
  ...
  setSort(valueSortColumn:any) // set sorting column
  {
      ...
      Cookie.set('currencies_sortAsc',this.sendingQueryForm.sortAsc);
      } else {
          this.sendingQueryForm.sortColumn=valueSortColumn;
          this.sendingQueryForm.sortAsc="asc";
          Cookie.set('currencies_sortAsc',"asc");
          Cookie.set('currencies_sortColumn',valueSortColumn);
      }
      this.getData();
  }
  onCompanySelection(){
    Cookie.set('currencies_companyId',this.sendingQueryForm.companyId);
    this.resetOptions();
    this.getCRUD_rights();
  }
  ...
  getCurrenciesNameById(id:string):string{
    let name:string = translate('docs.msg.not_set');
    if(this.receivedCurrenciesList){
      this.receivedCurrenciesList.forEach(a=>{
  ...
  deleteDocs(){
    ...
    this.clearCheckboxSelection();
        return this.http.post('/api/auth/deleteCurrencies', body) 
    .subscribe((data) => {   
  ...
  undeleteDocs(){
    const body = {"checked": this.checkedList.join()}; //join переводит из массива в строку
    this.clearCheckboxSelection();
     return this.http.post('/api/auth/undeleteCurrencies', body) 
    .subscribe(
        (data) => {   
          let result=data as any;
          switch(result){
  ...
  setDefaultCompany(){
    if(Cookie.get('currencies_companyId')=='0'||!this.companyIdInList(Cookie.get('currencies_companyId'))){
