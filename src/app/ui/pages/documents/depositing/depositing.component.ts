...
import { QueryFormService } from './get-depositing-table.service';
...
@Component({
  selector: 'app-depositing',
  templateUrl: './depositing.component.html',
  styleUrls: ['./depositing.component.css'],
  providers: [QueryFormService,LoadSpravService,Cookie,CommonUtilitesService] //+++
})
export class DepositingComponent implements OnInit {
  ...
  constructor(private queryFormService:   QueryFormService,
    ...
    private settingsDepositingDialogComponent: MatDialog,
    public deleteDialog: MatDialog,
    public dialogRef1: MatDialogRef<DepositingComponent>,
    ...
    ngOnInit() {
      ...
      if(Cookie.get('depositing_companyId')=='undefined' || Cookie.get('depositing_companyId')==null)     
        Cookie.set('depositing_companyId',this.sendingQueryForm.companyId); else this.sendingQueryForm.companyId=(Cookie.get('depositing_companyId')=="0"?"0":+Cookie.get('depositing_companyId'));
      if(Cookie.get('depositing_departmentId')=='undefined' || Cookie.get('depositing_departmentId')==null)  
        Cookie.set('depositing_departmentId',this.sendingQueryForm.departmentId); else this.sendingQueryForm.departmentId=(Cookie.get('depositing_departmentId')=="0"?"0":+Cookie.get('depositing_departmentId'));
      if(Cookie.get('depositing_sortAsc')=='undefined' || Cookie.get('depositing_sortAsc')==null)       
        Cookie.set('depositing_sortAsc',this.sendingQueryForm.sortAsc); else this.sendingQueryForm.sortAsc=Cookie.get('depositing_sortAsc');
      if(Cookie.get('depositing_sortColumn')=='undefined' || Cookie.get('depositing_sortColumn')==null)    
        Cookie.set('depositing_sortColumn',this.sendingQueryForm.sortColumn); else this.sendingQueryForm.sortColumn=Cookie.get('depositing_sortColumn');
      if(Cookie.get('depositing_offset')=='undefined' || Cookie.get('depositing_offset')==null)        
        Cookie.set('depositing_offset',this.sendingQueryForm.offset); else this.sendingQueryForm.offset=Cookie.get('depositing_offset');
      if(Cookie.get('depositing_result')=='undefined' || Cookie.get('depositing_result')==null)        
        Cookie.set('depositing_result',this.sendingQueryForm.result); else this.sendingQueryForm.result=Cookie.get('depositing_result');
...
// -------------------------------------- *** КОНЕЦ ПРАВ *** ------------------------------------
  ...
  setNumOfPages(){
    ...
    // this.createCheckedList();
    this.sendingQueryForm.offset=0;
    Cookie.set('depositing_result',this.sendingQueryForm.result);
    this.getData();
  }
  ...
  setPage(value:any) // set pagination
  {
    ...
    Cookie.set('depositing_offset',value);
    this.getData();
  }
  ...
  setSort(valueSortColumn:any) // set sorting column
  {
      ...
      Cookie.set('depositing_sortAsc',this.sendingQueryForm.sortAsc);
      } else {
          this.sendingQueryForm.sortColumn=valueSortColumn;
          this.sendingQueryForm.sortAsc="asc";
          Cookie.set('depositing_sortAsc',"asc");
          Cookie.set('depositing_sortColumn',valueSortColumn);
      }
      this.getData();
  }
  ...
  onCompanySelection(){
    ...
    Cookie.set('depositing_departmentId','0');
    // console.log('depositing_companyId - '+Cookie.get('depositing_companyId'));
    // console.log('depositing_departmentId - '+Cookie.get('depositing_departmentId'));
    this.sendingQueryForm.departmentId="0"; 
    this.resetOptions();
    this.getDepartmentsList();
  }
  onDepartmentSelection(){
    Cookie.set('depositing_departmentId',this.sendingQueryForm.departmentId);
    // console.log('depositing_companyId - '+Cookie.get('depositing_companyId'));
    // console.log('depositing_departmentId - '+Cookie.get('depositing_departmentId'));
    this.resetOptions();
    this.getData();
