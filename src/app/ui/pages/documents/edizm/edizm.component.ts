...
import { QueryFormService } from './get-edizm-table.service';
import { DeleteDialog } from 'src/app/ui/dialogs/deletedialog.component';
import { MessageDialog } from 'src/app/ui/dialogs/messagedialog.component';
import { FormGroup, FormControl } from '@angular/forms';
// import { SettingsEdizmDialogComponent } from 'src/app/modules/settings/settings-edizm-dialog/settings-edizm-dialog.component';
...
@Component({
  selector: 'app-edizm',
  templateUrl: './edizm.component.html',
  styleUrls: ['./edizm.component.css'],
  providers: [QueryFormService,LoadSpravService,Cookie]
})
export class EdizmComponent implements OnInit {
 ...
 visBtnDelete = false;
  ...
  constructor(private queryFormService:   QueryFormService,
    ...
    // private settingsEdizmDialogComponent: MatDialog,
    public deleteDialog: MatDialog,
    public dialogRef1: MatDialogRef<EdizmComponent>,) { }
    ...
    ngOnInit() {
      ...
      if(Cookie.get('edizm_companyId')=='undefined' || Cookie.get('edizm_companyId')==null)     
        Cookie.set('edizm_companyId',this.sendingQueryForm.companyId); else this.sendingQueryForm.companyId=(Cookie.get('edizm_companyId')=="0"?"0":+Cookie.get('edizm_companyId'));
      if(Cookie.get('edizm_sortAsc')=='undefined' || Cookie.get('edizm_sortAsc')==null)       
        Cookie.set('edizm_sortAsc',this.sendingQueryForm.sortAsc); else this.sendingQueryForm.sortAsc=Cookie.get('edizm_sortAsc');
      if(Cookie.get('edizm_sortColumn')=='undefined' || Cookie.get('edizm_sortColumn')==null)    
        Cookie.set('edizm_sortColumn',this.sendingQueryForm.sortColumn); else this.sendingQueryForm.sortColumn=Cookie.get('edizm_sortColumn');
      if(Cookie.get('edizm_offset')=='undefined' || Cookie.get('edizm_offset')==null)        
        Cookie.set('edizm_offset',this.sendingQueryForm.offset); else this.sendingQueryForm.offset=Cookie.get('edizm_offset');
      if(Cookie.get('edizm_result')=='undefined' || Cookie.get('edizm_result')==null)        
        Cookie.set('edizm_result',this.sendingQueryForm.result); else this.sendingQueryForm.result=Cookie.get('edizm_result');
...
// -------------------------------------- *** КОНЕЦ ПРАВ *** ------------------------------------
  ...
  setNumOfPages(){
    ...
    this.createCheckedList();
    this.sendingQueryForm.offset=0;
    Cookie.set('edizm_result',this.sendingQueryForm.result);
    this.getData();
  }
  ...
  setSort(valueSortColumn:any) // set sorting column
  {
      ...
      Cookie.set('edizm_sortAsc',this.sendingQueryForm.sortAsc);
      } else {
          this.sendingQueryForm.sortColumn=valueSortColumn;
          this.sendingQueryForm.sortAsc="asc";
          Cookie.set('edizm_sortAsc',"asc");
          Cookie.set('edizm_sortColumn',valueSortColumn);
      }
      this.getData();
  }
  onCompanySelection(){
  ...
  deleteDocs(){
    ...
    this.clearCheckboxSelection();
        return this.http.post('/api/auth/deleteEdizm', body) 
    .subscribe((data) => {   
      ...
      switch(result){
        ...
        case -1:{this.MessageDialog.open(MessageDialog,{width:'400px',data:{head:'Внимание!',message:"Недостаточно прав для операции удаления"}});break;}
      }
    },error => {console.log(error);this.MessageDialog.open(MessageDialog,{width:'400px',data:{head:'Ошибка!',message:error.error}})},);
  }
  ...
  setDefaultCompany(){
    if(Cookie.get('edizm_companyId')=='0'){
  ...
  doFilterCompaniesList(){
    ...
    // openDialogSettings() { 
    //   const dialogSettings = this.settingsEdizmDialogComponent.open(SettingsEdizmDialogComponent, {
    //     maxWidth: '95vw',
    //     maxHeight: '95vh',
    //     // height: '680px',
    //     width: '400px', 
  ...
  undeleteDocs(){
    const body = {"checked": this.checkedList.join()}; //join переводит из массива в строку
    this.clearCheckboxSelection();
     return this.http.post('/api/auth/undeleteEdizm', body) 
    .subscribe(
        (data) => {   
          let result=data as any;
          switch(result){
