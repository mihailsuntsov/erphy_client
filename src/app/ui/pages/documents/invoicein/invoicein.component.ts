...
import { QueryFormService } from './get-invoicein-table.service';
...
export class InvoiceinComponent implements OnInit {
  ...
  constructor(private queryFormService:   QueryFormService,
    ...
    private settingsInvoiceinDialogComponent: MatDialog,
    public deleteDialog: MatDialog,
    public dialogRef1: MatDialogRef<InvoiceinComponent>,
    ...
    ngOnInit() {
      ...
      if(Cookie.get('invoicein_companyId')=='undefined' || Cookie.get('invoicein_companyId')==null)     
        Cookie.set('invoicein_companyId',this.sendingQueryForm.companyId); else this.sendingQueryForm.companyId=(Cookie.get('invoicein_companyId')=="0"?"0":+Cookie.get('invoicein_companyId'));
      if(Cookie.get('invoicein_departmentId')=='undefined' || Cookie.get('invoicein_departmentId')==null)  
        Cookie.set('invoicein_departmentId',this.sendingQueryForm.departmentId); else this.sendingQueryForm.departmentId=(Cookie.get('invoicein_departmentId')=="0"?"0":+Cookie.get('invoicein_departmentId'));
      if(Cookie.get('invoicein_sortAsc')=='undefined' || Cookie.get('invoicein_sortAsc')==null)       
        Cookie.set('invoicein_sortAsc',this.sendingQueryForm.sortAsc); else this.sendingQueryForm.sortAsc=Cookie.get('invoicein_sortAsc');
      if(Cookie.get('invoicein_sortColumn')=='undefined' || Cookie.get('invoicein_sortColumn')==null)    
        Cookie.set('invoicein_sortColumn',this.sendingQueryForm.sortColumn); else this.sendingQueryForm.sortColumn=Cookie.get('invoicein_sortColumn');
      if(Cookie.get('invoicein_offset')=='undefined' || Cookie.get('invoicein_offset')==null)        
        Cookie.set('invoicein_offset',this.sendingQueryForm.offset); else this.sendingQueryForm.offset=Cookie.get('invoicein_offset');
      if(Cookie.get('invoicein_result')=='undefined' || Cookie.get('invoicein_result')==null)        
        Cookie.set('invoicein_result',this.sendingQueryForm.result); else this.sendingQueryForm.result=Cookie.get('invoicein_result');
...
// -------------------------------------- *** КОНЕЦ ПРАВ *** ------------------------------------
  ...
  setNumOfPages(){
    ...
    this.createCheckedList();
    this.sendingQueryForm.offset=0;
    Cookie.set('invoicein_result',this.sendingQueryForm.result);
    this.getData();
  }
  ...
  setPage(value:any) // set pagination
  {
    ...
    Cookie.set('invoicein_offset',value);
    this.getData();
  }
  ...
  setSort(valueSortColumn:any) // set sorting column
  {
      ...
      Cookie.set('invoicein_sortAsc',this.sendingQueryForm.sortAsc);
      } else {
          this.sendingQueryForm.sortColumn=valueSortColumn;
          this.sendingQueryForm.sortAsc="asc";
          Cookie.set('invoicein_sortAsc',"asc");
          Cookie.set('invoicein_sortColumn',valueSortColumn);
      }
      this.getData();
  }
  onCompanySelection(){
    Cookie.set('invoicein_companyId',this.sendingQueryForm.companyId);
    Cookie.set('invoicein_departmentId','0');
    // console.log('invoicein_companyId - '+Cookie.get('invoicein_companyId'));
    // console.log('invoicein_departmentId - '+Cookie.get('invoicein_departmentId'));
    this.sendingQueryForm.departmentId="0"; 
    this.resetOptions();
    this.getDepartmentsList();
  }
  onDepartmentSelection(){
    Cookie.set('invoicein_departmentId',this.sendingQueryForm.departmentId);
    // console.log('invoicein_companyId - '+Cookie.get('invoicein_companyId'));
    // console.log('invoicein_departmentId - '+Cookie.get('invoicein_departmentId'));
    this.resetOptions();
    this.getData();
  ...
  undeleteDocs(){
    ...
    this.clearCheckboxSelection();
    return this.http.post('/api/auth/undeleteInvoicein', body) 
    .subscribe(
      (data) => {   
  ...
  deleteDocs(){
    const body = {"checked": this.checkedList.join()}; //join переводит из массива в строку
    this.clearCheckboxSelection();
  return this.http.post('/api/auth/deleteInvoicein', body) 
  .subscribe((data) => {   
      let result=data as any;
      switch(result.result){
        ...
        case 3:{let numbers:string='';
          for(var i=0;i<result.docs.length;i++){numbers=numbers+' <a href="ui/invoiceindoc/'+result.docs[i].id+'">'+result.docs[i].doc_number+'</a>';}
  ...
  setDefaultCompany(){
    if(Cookie.get('invoicein_companyId')=='0'||!this.companyIdInList(Cookie.get('invoicein_companyId'))){
