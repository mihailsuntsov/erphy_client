...
interface InvoiceoutProductTable { //интерфейс для формы, массив из которых будет содержать форма invoiceoutProductTable, входящая в formBaseInformation
  id: number;
  row_id: number;
  product_id: number;
...
interface DocResponse {//интерфейс для получения ответа в методе getInvoiceoutValuesById
  id: number;
  company: string;
  company_id: number;
  department: string;
...
export class InvoiceoutDocComponent implements OnInit {
  ...
  constructor(private activateRoute: ActivatedRoute,
    ...
    private _fb: UntypedFormBuilder, //чтобы билдить группу форм invoiceoutProductTable
  ...
  ngOnInit() {
    this.formBaseInformation = new UntypedFormGroup({
      ...
      invoiceout_date: new UntypedFormControl      ('',[Validators.required]),
    ...
    this.formLinkedDocs = new UntypedFormGroup({
      ...
      department_id: new UntypedFormControl      (null,[Validators.required]),
      description: new UntypedFormControl        ('',[]),
      linked_doc_id: new UntypedFormControl      (null,[]),//id связанного документа (в данном случае ЭТОГО документа)
      ...
      shipment_date: new UntypedFormControl      ('',[]), // планируемая дата отгрузки
  ...
  getCRUD_rights(){
    this.allowToCreateAllCompanies = this.permissionsSet.some(         function(e){return(e==405)});
    this.allowToCreateMyCompany = this.permissionsSet.some(            function(e){return(e==406)});
    this.allowToCreateMyDepartments = this.permissionsSet.some(        function(e){return(e==407)});
    this.allowToViewAllCompanies = this.permissionsSet.some(           function(e){return(e==412)});
    this.allowToViewMyCompany = this.permissionsSet.some(              function(e){return(e==413)});
    this.allowToViewMyDepartments = this.permissionsSet.some(          function(e){return(e==414)});
    this.allowToViewMyDocs = this.permissionsSet.some(                 function(e){return(e==415)});
    this.allowToUpdateAllCompanies = this.permissionsSet.some(         function(e){return(e==416)});
    this.allowToUpdateMyCompany = this.permissionsSet.some(            function(e){return(e==417)});
    this.allowToUpdateMyDepartments = this.permissionsSet.some(        function(e){return(e==418)});
    this.allowToUpdateMyDocs = this.permissionsSet.some(               function(e){return(e==419)});
    this.allowToCompleteAllCompanies = this.permissionsSet.some(       function(e){return(e==420)});
    this.allowToCompleteMyCompany = this.permissionsSet.some(          function(e){return(e==421)});
    this.allowToCompleteMyDepartments = this.permissionsSet.some(      function(e){return(e==422)});
    this.allowToCompleteMyDocs = this.permissionsSet.some(             function(e){return(e==423)});
  ...
  getStatusesList(){
    ...
    this.loadSpravService.getStatusList(this.formBaseInformation.get('company_id').value,31) //31 - id документа из таблицы documents
            .subscribe(
  ...
  getSettings(){
    let result:any;
    this.http.get('/api/auth/getSettingsInvoiceout')
      .subscribe(
          data => { 
            result=data as any;
  ...
  getDocumentValuesById(onlyBaseInformation?:boolean){
    this.http.get('/api/auth/getInvoiceoutValuesById?id='+ this.id)
        .subscribe(
            data => { 
  ...
  getTotalProductCount() {//бежим по столбцу product_count и складываем (аккумулируем) в acc начиная с 0 значения этого столбца
    ...
    return (this.formBaseInformation.value.invoiceoutProductTable.map(t => +t.product_count).reduce((acc, value) => acc + value, 0)).toFixed(3).replace(".000", "").replace(".00", "");
  }
  getTotalSumPrice() {//бежим по столбцу product_sumprice и складываем (аккумулируем) в acc начиная с 0 значения этого столбца
    this.getProductsTable();
    return (this.formBaseInformation.value.invoiceoutProductTable.map(t => +t.product_sumprice).reduce((acc, value) => acc + value, 0)).toFixed(2);
  }
  ...
  getFormIngexByProductId(productId:number):number{
    ...
    this.formBaseInformation.value.invoiceoutProductTable.map(i => 
        {
        if(+i['product_id']==productId){retIndex=formIndex}
        formIndex++;
        });return retIndex;}

  formingProductRowFromApiResponse(row: InvoiceoutProductTable) {
  ...
  hideOrShowNdsColumn(){
    if(this.formBaseInformation.get('nds').value){
      this.displayedColumns = ['select','invoiceout_date','product_count','edizm','product_price','product_sumprice','reserved_current','available','total','reserved','shipped','price_type','nds','department',/*'id','row_id','indx',*/'delete'];
    } else {
      this.displayedColumns = ['select','invoiceout_date','product_count','edizm','product_price','product_sumprice','reserved_current','available','total','reserved','shipped','price_type','department',/*'id','row_id','indx',*/'delete'];
    }
  }
//????????????????
  // getControlTablefield(){
  //   const control = <FormArray>this.formBaseInformation.get('invoiceoutProductTable');
  //   return control;
  // }
      ...
      this.http.post('/api/auth/insertInvoiceout', this.formBaseInformation.value)
        ...
        (data) => {
          ...
          if (this.createdDocId==0){// 0 возвращает если не удалось сохранить изза превышения количества покупаемого товара над доступным количеством
            ...
            this.MessageDialog.open(MessageDialog,{width:'400px',data:{head:translate('docs.msg.error'),message:translate('docs.msg.crte_doc_err',{name:translate('docs.docs.invoiceout')})}});
          } else {// Документ успешно создался в БД 
            this.openSnackBar(translate('docs.msg.doc_crtd_succ',{name:translate('docs.docs.invoiceout')}), translate('docs.msg.close'));
            //действия после создания нового документа Счёт покупателю (это самый последний этап)
            this.afterCreateInvoiceout(withReceipt);
          }
        },
        error => {console.log(error);this.MessageDialog.open(MessageDialog,{width:'400px',data:{head:translate('docs.msg.error'),message:error.error}});},
        );
  ...
  // Действия после создания нового документа Счёт покупателю (это самый последний этап).
  afterCreateInvoiceout(withReceipt:boolean){// с true запрос придет при отбиваемом в данный момент чеке
    // Сначала обживаем текущий документ:
    this.id=+this.createdDocId;
    this._router.navigate(['/ui/invoiceoutdoc', this.id]);
      ...
      .subscribe( 
          ...
          {   
            ...
            switch(result){
              ...
              case 1:{// Успешно
                this.openSnackBar(translate('docs.msg.cnc_com_succs',{name:translate('docs.docs.invoiceout')}), translate('docs.msg.close'));
                // this.getLinkedDocsScheme(true);//загрузка диаграммы связанных документов
          ...
          {   
            ...
            switch(result){
              case null:{// null возвращает если не удалось создать документ из-за ошибки
                this.MessageDialog.open(MessageDialog,{width:'400px',data:{head:translate('docs.msg.error'),message:translate('docs.msg.error_of') + (complete?translate('docs.msg._of_comp'):translate('docs.msg._of_save')) + translate('docs.msg._of_doc',{name:translate('docs.docs.invoiceout')})}});
                break;
              }
              case -1:{//недостаточно прав
                this.MessageDialog.open(MessageDialog,{width:'400px',data:{head:translate('docs.msg.error'),message:translate('docs.msg.ne_perm_oper') + (complete?translate('docs.msg._of_comp'):translate('docs.msg._of_save')) + translate('docs.msg._of_doc',{name:translate('docs.docs.invoiceout')})}});
                break;
              }
              ...
              default:{// Успешно
                this.openSnackBar(translate('docs.msg.doc_name',{name:translate('docs.docs.invoiceout')}) + (complete?translate('docs.msg.completed'):translate('docs.msg.saved')), translate('docs.msg.close'));
                this.getLinkedDocsScheme(true);//загрузка диаграммы связанных документов
                if(complete) {
...
// при нажатии на кнопку Скачать
  ...
  getProductsTable(){
    const control = <UntypedFormArray>this.formBaseInformation.get('invoiceoutProductTable');
    control.clear();
  ...
  openDialogSettings() { 
    ...
    dialogSettings.afterClosed().subscribe(result => {
      if(result){
        ...
        if(result.get('invoiceout_date')) this.settingsForm.get('invoiceout_date').setValue(result.get('invoiceout_date').value);
...
//устанавливает цвет статуса (используется для цветовой индикации статусов)
  ...
  goToNewDocument(){
      ...
      this.formBaseInformation.get('cagent_id').setValue(null);
      this.formBaseInformation.get('cagent').setValue('');
      this.formBaseInformation.get('invoiceout_date').setValue('');
      this.formBaseInformation.get('description').setValue('');
      this.searchCagentCtrl.reset();

      setTimeout(() => { this.productSearchAndTableComponent.hideOrShowNdsColumn();}, 1000);
      this.actionsBeforeGetChilds=0;
...
//**********************************************************************************************************************************************/  
  ...
  createLinkedDoc(docname:string){// принимает аргументы: Return
    ...
    if(canCreateLinkedDoc.can){
      ...
      this.formLinkedDocs.get('description').setValue(translate('docs.msg.created_from')+translate('docs.docs.invoiceout')+' '+translate('docs.top.number')+this.formBaseInformation.get('doc_number').value);
...
// забирает таблицу товаров из дочернего компонента и помещает ее в форму, предназначенную для создания дочерних документов
  getProductsTableLinkedDoc(docname:string){
    ...
    if(docname=='Shipment'){// из создающихся из данногод документа связанных документов таблица товара есть только у Отгрузки
      ...
      const control = <UntypedFormArray>this.formLinkedDocs.get(tableName);
      control.clear();
      this.productSearchAndTableComponent.getProductTable().forEach(row=>{
