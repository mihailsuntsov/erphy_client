...
import { QueryFormService } from './get-jobtitles-table.service';
...
export class JobtitlesComponent implements OnInit {
  ...
  receivedJobtitlesList: idAndName [] = [];//массив для получения списка расходов
  ...
  constructor(private queryFormService:   QueryFormService,
    ...
    public dialogRef1: MatDialogRef<JobtitlesComponent>,
    ...
    ngOnInit() {
      ...
      if(Cookie.get('jobtitles_companyId')=='undefined' || Cookie.get('jobtitles_companyId')==null)     
        Cookie.set('jobtitles_companyId',this.sendingQueryForm.companyId); else this.sendingQueryForm.companyId=(Cookie.get('jobtitles_companyId')=="0"?"0":+Cookie.get('jobtitles_companyId'));
      if(Cookie.get('jobtitles_sortAsc')=='undefined' || Cookie.get('jobtitles_sortAsc')==null)       
        Cookie.set('jobtitles_sortAsc',this.sendingQueryForm.sortAsc); else this.sendingQueryForm.sortAsc=Cookie.get('jobtitles_sortAsc');
      if(Cookie.get('jobtitles_sortColumn')=='undefined' || Cookie.get('jobtitles_sortColumn')==null)    
        Cookie.set('jobtitles_sortColumn',this.sendingQueryForm.sortColumn); else this.sendingQueryForm.sortColumn=Cookie.get('jobtitles_sortColumn');
      if(Cookie.get('jobtitles_offset')=='undefined' || Cookie.get('jobtitles_offset')==null)        
        Cookie.set('jobtitles_offset',this.sendingQueryForm.offset); else this.sendingQueryForm.offset=Cookie.get('jobtitles_offset');
      if(Cookie.get('jobtitles_result')=='undefined' || Cookie.get('jobtitles_result')==null)        
        Cookie.set('jobtitles_result',this.sendingQueryForm.result); else this.sendingQueryForm.result=Cookie.get('jobtitles_result');
...
// -------------------------------------- *** КОНЕЦ ПРАВ *** ------------------------------------
  ...
  setNumOfPages(){
    ...
    this.createCheckedList();
    this.sendingQueryForm.offset=0;
    Cookie.set('jobtitles_result',this.sendingQueryForm.result);
    this.getData();
  }
  ...
  setPage(value:any) // set pagination
  {
    ...
    Cookie.set('jobtitles_offset',value);
    this.getData();
  }
  ...
  setSort(valueSortColumn:any) // set sorting column
  {
      ...
      Cookie.set('jobtitles_sortAsc',this.sendingQueryForm.sortAsc);
      } else {
          this.sendingQueryForm.sortColumn=valueSortColumn;
          this.sendingQueryForm.sortAsc="asc";
          Cookie.set('jobtitles_sortAsc',"asc");
          Cookie.set('jobtitles_sortColumn',valueSortColumn);
      }
      this.getData();
  }
  onCompanySelection(){
    Cookie.set('jobtitles_companyId',this.sendingQueryForm.companyId);
    this.resetOptions();
    this.getCRUD_rights();
  }
  ...
  getJobtitlesNameById(id:string):string{
    let name:string = translate('docs.msg.not_set');
    if(this.receivedJobtitlesList){
      this.receivedJobtitlesList.forEach(a=>{
  ...
  undeleteDocs(){
    ...
    this.clearCheckboxSelection();
      return this.http.post('/api/auth/undeleteJobtitle', body) 
    .subscribe(
        (data) => {   
  ...
  deleteDocs(){
    const body = {"checked": this.checkedList.join()}; //join переводит из массива в строку
    this.clearCheckboxSelection();
    return this.http.post('/api/auth/deleteJobtitle', body) 
      .subscribe(
        (data) => {  
          let result=data as any;
          switch(result){ //+++
  ...
  setDefaultCompany(){
    if(Cookie.get('jobtitles_companyId')=='0'||!this.companyIdInList(Cookie.get('jobtitles_companyId'))){
