...
interface dockResponse {//интерфейс для получения ответа в методе getKassaValuesById
  id: number;
...
@Component({
  selector: 'app-kassa-dock',
  templateUrl: './kassa-dock.component.html',
  styleUrls: ['./kassa-dock.component.css'],
  providers: [LoadSpravService,KkmAtolService]
})
export class KassaDockComponent implements OnInit {
  ...
  createdDockId: number;// id созданного документа
...
//---------------------------------------------------------------------------------------------------------------------------------------
  ...
  getSetOfPermissions(){
    const body = {"documentId": 24};//24= "Кассы онлайн"
...
// -------------------------------------- *** КОНЕЦ ПРАВ *** ------------------------------------
  ...
  getDepartmentsList(newDock:boolean){
    ...
    if (newDock) this.formBaseInformation.get('department_id').setValue('');
    this.loadSpravService.getDepartmentsListByCompanyId(this.formBaseInformation.get('company_id').value,false)
            .subscribe(
                (data) => {this.receivedDepartmentsList=data as any [];
                    ...
                    if (newDock) this.setDefaultDepartment();},
                error => console.log(error)
            );
  }
  setDefaultDepartment(){
