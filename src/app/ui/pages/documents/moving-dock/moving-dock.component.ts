...
interface DockResponse {//интерфейс для получения ответа в методе getMovingValuesById
  id: number;
  company: string;
  company_id: string;
...
@Component({
  selector: 'app-moving-dock',
  templateUrl: './moving-dock.component.html',
  styleUrls: ['./moving-dock.component.css'],
  providers: [LoadSpravService,
    {provide: MAT_DATE_LOCALE, useValue: 'ru'},
    {provide: DateAdapter, useClass: MomentDateAdapter, deps: [MAT_DATE_LOCALE]},
    {provide: MAT_DATE_FORMATS, useValue: MY_FORMATS},]
})
export class MovingDockComponent implements OnInit {
...
    this.http.get('/api/auth/getMovingValuesById?id='+ this.id)
  ...
  afterCreateMoving(){
      ...
      this._router.navigate(['/ui/movingdock', this.id]);
  ...
  goToNewDocument(){
    this._router.navigate(['ui/movingdock',0]);
    this.id=0;
...
//     });
//   }
//   formingProductRowLinkedDoc(row: MovingProductTable, dockname:string) {
//     return this._fb.group({
//       product_id: new FormControl (row.product_id,[]),
//       product_count: new FormControl (row.product_count,[]),
