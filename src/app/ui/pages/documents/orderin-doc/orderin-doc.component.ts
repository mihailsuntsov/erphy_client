...
export class OrderinDocComponent implements OnInit {
  ...
  constructor(private activateRoute: ActivatedRoute,
    ...
    private _fb: FormBuilder, //чтобы билдить группу форм orderinProductTable    
  ...
  ngOnInit(): void {
    this.formBaseInformation = new FormGroup({
      ...
      moving_type: new FormControl              ('',[]),// тип перемещения (из кассы предприятия, расч. счёта, кассы ККМ)
      boxoffice_from_id: new FormControl        ('',[]), // id кассы предприятия
      payment_account_from_id: new FormControl  ('',[]), // id расчётного счёта
      kassa_from_id: new FormControl            ('',[]),// id кассы ККМ откуда делаем выемку
      boxoffice_from: new FormControl           ('',[]), // касса предприятия
      payment_account_from: new FormControl     ('',[]), // расчётный счёт
      kassa_from: new FormControl               ('',[]),// касса ККМ откуда делаем выемку
      ...
      boxoffice: new FormControl                ('',[]),// касса в которую перемещаем ден. средства
      internal: new FormControl                 (false,[]), // внутренний платеж
      withdrawal_id: new FormControl            ('',[]),             // id выемки, из которой поступили средства
      ...
      withdrawal: new FormControl               ('',[]),             // номер выемки, из которой поступили средства
...
//-------------------------------------------------------------------------------
  ...
  getCRUD_rights(permissionsSet:any[]){
    ...
    this.allowToCompleteAllCompanies = permissionsSet.some(       function(e){return(e==484)});
    this.allowToCompleteMyCompany = permissionsSet.some(          function(e){return(e==485)});
      ...
      (data) => {
                  ...
                  switch(this.createdDocId){
                    case null:{// null возвращает если не удалось создать документ из-за ошибки
                      this.MessageDialog.open(MessageDialog,{width:'400px',data:{head:'Ошибка!',message:"Ошибка создания документа Приходный ордер"}});
                      break;
                    }
                    ...
                    default:{// Приходный ордер успешно создалась в БД 
                      this.openSnackBar("Документ \"Приходный ордер\" успешно создан", "Закрыть");
    ...
    if(!notShowDialog){//notShowDialog=false - показывать диалог
      const dialogRef = this.ConfirmDialog.open(ConfirmDialog, {
        width: '400px',data:{
          ...
          warning: 'Вы хотите провести данный приходный ордер?',
    ...
    this.http.post('/api/auth/updateOrderin',  this.formBaseInformation.value)
      .subscribe(
          ...
          {   
            ...
            switch(result){
              case null:{// null возвращает если не удалось создать документ из-за ошибки
                this.MessageDialog.open(MessageDialog,{width:'400px',data:{head:'Ошибка!',message:"Ошибка "+ (complete?"проведения":"сохренения") + " документа \"Приходный ордер\""}});
                break;
              }
              case -1:{//недостаточно прав
                this.MessageDialog.open(MessageDialog,{width:'400px',data:{head:'Ошибка!',message:"Недостаточно прав для создания документа \"Приходный ордер\""}});
  ...
  getWithdrawalListByKassaId(){
    if(+this.formBaseInformation.get('kassa_from_id').value>0 && !this.formBaseInformation.get('is_completed').value){
      ...
      this.http.get('/api/auth/getWithdrawalList?kassa_id='+this.formBaseInformation.get('kassa_from_id').value).subscribe(
        ...
        error => {this.withdrawalListLoading=false;console.log(error);this.MessageDialog.open(MessageDialog,{width:'400px',data:{head:'Ошибка!',message:error}})}
      );
    }
  }
