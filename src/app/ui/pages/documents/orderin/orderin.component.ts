...
import { QueryFormService } from './get-orderin-table.service';
...
export class OrderinComponent implements OnInit {
 ...
 visBtnDelete = false;
  ...
  constructor(private queryFormService:   QueryFormService,
    ...
    private settingsOrderinDialogComponent: MatDialog,
    public deleteDialog: MatDialog,
    public dialogRef1: MatDialogRef<OrderinComponent>,
    ...
    ngOnInit() {
      ...
      if(Cookie.get('orderin_companyId')=='undefined' || Cookie.get('orderin_companyId')==null)     
        Cookie.set('orderin_companyId',this.sendingQueryForm.companyId); else this.sendingQueryForm.companyId=(Cookie.get('orderin_companyId')=="0"?"0":+Cookie.get('orderin_companyId'));
      if(Cookie.get('orderin_sortAsc')=='undefined' || Cookie.get('orderin_sortAsc')==null)       
        Cookie.set('orderin_sortAsc',this.sendingQueryForm.sortAsc); else this.sendingQueryForm.sortAsc=Cookie.get('orderin_sortAsc');
      if(Cookie.get('orderin_sortColumn')=='undefined' || Cookie.get('orderin_sortColumn')==null)    
        Cookie.set('orderin_sortColumn',this.sendingQueryForm.sortColumn); else this.sendingQueryForm.sortColumn=Cookie.get('orderin_sortColumn');
      if(Cookie.get('orderin_offset')=='undefined' || Cookie.get('orderin_offset')==null)        
        Cookie.set('orderin_offset',this.sendingQueryForm.offset); else this.sendingQueryForm.offset=Cookie.get('orderin_offset');
      if(Cookie.get('orderin_result')=='undefined' || Cookie.get('orderin_result')==null)        
        Cookie.set('orderin_result',this.sendingQueryForm.result); else this.sendingQueryForm.result=Cookie.get('orderin_result');
  ...
  getCRUD_rights(permissionsSet:any[]){
    this.allowToCreateAllCompanies = permissionsSet.some(         function(e){return(e==476)});
    this.allowToCreateMyCompany = permissionsSet.some(            function(e){return(e==477)});
    this.allowToDeleteAllCompanies = permissionsSet.some(         function(e){return(e==478)});
    this.allowToDeleteMyCompany = permissionsSet.some(            function(e){return(e==479)});
    this.allowToViewAllCompanies = permissionsSet.some(           function(e){return(e==480)});
    this.allowToViewMyCompany = permissionsSet.some(              function(e){return(e==481)});
    this.allowToUpdateAllCompanies = permissionsSet.some(         function(e){return(e==482)});
    this.allowToUpdateMyCompany = permissionsSet.some(            function(e){return(e==483)});
    this.getData();
  }
...
// -------------------------------------- *** КОНЕЦ ПРАВ *** ------------------------------------
  ...
  setNumOfPages(){
    ...
    this.createCheckedList();
    this.sendingQueryForm.offset=0;
    Cookie.set('orderin_result',this.sendingQueryForm.result);
    this.getData();
  }
  ...
  setPage(value:any) // set pagination
  {
    ...
    Cookie.set('orderin_offset',value);
    this.getData();
  }
  ...
  setSort(valueSortColumn:any) // set sorting column
  {
      ...
      Cookie.set('orderin_sortAsc',this.sendingQueryForm.sortAsc);
      } else {
          this.sendingQueryForm.sortColumn=valueSortColumn;
          this.sendingQueryForm.sortAsc="asc";
          Cookie.set('orderin_sortAsc',"asc");
          Cookie.set('orderin_sortColumn',valueSortColumn);
      }
      this.getData();
  }
  onCompanySelection(){
    Cookie.set('orderin_companyId',this.sendingQueryForm.companyId);
    this.resetOptions();
    this.getData();
  }
  clickBtnDelete(): void {
  ...
  undeleteDocs(){
    ...
    this.clearCheckboxSelection();
    return this.http.post('/api/auth/undeleteOrderin', body) 
    .subscribe(
        (data) => {
  ...
  deleteDocs(){
    const body = {"checked": this.checkedList.join()}; //join переводит из массива в строку
    this.clearCheckboxSelection();
    return this.http.post('/api/auth/deleteOrderin', body) 
    .subscribe((data) => {  
      let result=data as any;
      switch(result.result){
        ...
        case 3:{let numbers:string='';
          for(var i=0;i<result.docs.length;i++){numbers=numbers+' <a href="ui/orderindoc/'+result.docs[i].id+'">'+result.docs[i].doc_number+'</a>';}
  ...
  setDefaultCompany(){
    if(Cookie.get('orderin_companyId')=='0'||!this.companyIdInList(Cookie.get('orderin_companyId'))){
