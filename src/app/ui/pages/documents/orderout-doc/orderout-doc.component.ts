...
export class OrderoutDocComponent implements OnInit {
  ...
  kassaList:any[]=[];  // список касс ККМ по отделениям, которые (отделения) привязаны к кассе предприятия boxoffice_id
  ...
  constructor(private activateRoute: ActivatedRoute,
    ...
    private _fb: UntypedFormBuilder, //чтобы билдить группу форм orderoutProductTable    
    public SettingsOrderoutDialogComponent: MatDialog,
  ...
  ngOnInit(): void {
    this.formBaseInformation = new UntypedFormGroup({
      ...
      kassa_to_id: new UntypedFormControl              ('',[]),// id кассы ККМ, в которую делаем внесение
      kassa_to: new UntypedFormControl                 ('',[]),// касса ККМ, в которую делаем внесение
      payment_account_to_id: new UntypedFormControl    ('',[]),
      boxoffice_to_id: new UntypedFormControl          ('',[]),
      status_color: new UntypedFormControl             ('',[]),
      status_description: new UntypedFormControl       ('',[]),
      kassa_department_id: new UntypedFormControl      ('',[]),      
      kassa_department: new UntypedFormControl         ('',[]),
    ...
    this.formLinkedDocs = new UntypedFormGroup({
      ...
      orderout_id: new UntypedFormControl         ('',[]), //для счёта фактуры выданного
      ...
      summ: new UntypedFormControl               ('',[]), // сумма   
      // параметры для входящих ордеров и платежей
      internal: new UntypedFormControl           ('',[]), // внутренний платеж  
      moving_type: new UntypedFormControl              ('',[]), // тип перевода  -  на: кассу предприятия - boxoffice, счёт - account, касса ККМ - kassa 
      boxoffice_from_id: new UntypedFormControl  (null,[]), // из какой кассы предприятия переводим
      ...
      kassa_id: new UntypedFormControl           ('',[]), // id кассы ККМ, в которую переводят
...
//-------------------------------------------------------------------------------
  ...
  getStatusesList(){
    ...
    this.loadSpravService.getStatusList(this.formBaseInformation.get('company_id').value,36) //36 - id документа из таблицы documents
            .subscribe(
  ...
  doFilterCompaniesList(){
    ...
    if(+this.id==0)//!!!!! отсюда загружаем настройки только если документ новый. Если уже создан - настройки грузятся из get<Document>ValuesById
      this.getSettings(); // настройки документа Расходный ордер
  }
  ...
  createNewDocument(){
    ...
    this.http.post('/api/auth/insertOrderout', this.formBaseInformation.value)
      .subscribe(
      (data) => {
                  this.actionsBeforeGetChilds=0;
  ...
  afterCreateOrderout(){
      ...
      this._router.navigate(['/ui/orderoutdoc', this.id]);
  ...
  updateDocument(complete?:boolean){ 
    ...
    this.http.post('/api/auth/updateOrderout',  this.formBaseInformation.value)
      .subscribe(
          (data) => 
          {   
            if(complete){
...
openDialogAddFiles() {
  ...
  dialogRef.afterClosed().subscribe(result => {
    ...
    if(result)this.addFilesToOrderout(result);
  });
}
...
loadFilesInfo(){//                                     загружает информацию по прикрепленным файлам
    return this.http.get('/api/auth/getListOfOrderoutFiles?id='+this.id) 
          .subscribe(
              (data) => {  
...
addFilesToOrderout(filesIds: number[]){
  ...
          return this.http.post('/api/auth/addFilesToOrderout', body) 
            .subscribe(
                (data) => {  
                  this.loadFilesInfo();
...
deleteFile(id:number){
  ...
  return this.http.post('/api/auth/deleteOrderoutFile',body)
  .subscribe(
      (data) => {   
                  this.loadFilesInfo();
...
//**********************************************************************************************************************************************/  
  ...
  createLinkedDoc(docname:string){// принимает аргументы: Return
    ...
    if(canCreateLinkedDoc.can){
      ...
      if(docname=='Paymentin'||docname=='Orderin'){this.formLinkedDocs.get('internal').setValue(this.expenditureType=='moving');
        this.formLinkedDocs.get('moving_type').setValue('boxoffice');// тип перевода  -  из : кассы - boxoffice, счёта - account, кассы ККМ - kassa
        this.formLinkedDocs.get('boxoffice_from_id').setValue(this.formBaseInformation.get('boxoffice_id').value); // id кассы предприятия, из которой переводят 
      ...
      if(docname=='Ordersup'){// Заказ поставщику для Расходного ордера является родительским, но может быть создан из Расходного ордера (Заказ поставщику будет выше по иерархии в диаграмме связей)
...
//*****************************************************************************************************************************************/
  ...
  getFormIngexByProductId(productId:number):number{
    ...
    this.formBaseInformation.value.orderoutProductTable.map(i => 
      {
