...
import { QueryFormService } from './get-orderout-table.service';
...
@Component({
  selector: 'app-orderout',
  templateUrl: './orderout.component.html',
  styleUrls: ['./orderout.component.css'],
  providers: [QueryFormService,LoadSpravService,Cookie,CommonUtilitesService] //+++
})
export class OrderoutComponent implements OnInit {
 ...
 visBtnDelete = false;
  ...
  constructor(private queryFormService:   QueryFormService,
    ...
    private settingsOrderoutDialogComponent: MatDialog,
    public deleteDialog: MatDialog,
    public dialogRef1: MatDialogRef<OrderoutComponent>,
    ...
    ngOnInit() {
      ...
      if(Cookie.get('orderout_companyId')=='undefined' || Cookie.get('orderout_companyId')==null)     
        Cookie.set('orderout_companyId',this.sendingQueryForm.companyId); else this.sendingQueryForm.companyId=(Cookie.get('orderout_companyId')=="0"?"0":+Cookie.get('orderout_companyId'));
      if(Cookie.get('orderout_sortAsc')=='undefined' || Cookie.get('orderout_sortAsc')==null)       
        Cookie.set('orderout_sortAsc',this.sendingQueryForm.sortAsc); else this.sendingQueryForm.sortAsc=Cookie.get('orderout_sortAsc');
      if(Cookie.get('orderout_sortColumn')=='undefined' || Cookie.get('orderout_sortColumn')==null)    
        Cookie.set('orderout_sortColumn',this.sendingQueryForm.sortColumn); else this.sendingQueryForm.sortColumn=Cookie.get('orderout_sortColumn');
      if(Cookie.get('orderout_offset')=='undefined' || Cookie.get('orderout_offset')==null)        
        Cookie.set('orderout_offset',this.sendingQueryForm.offset); else this.sendingQueryForm.offset=Cookie.get('orderout_offset');
      if(Cookie.get('orderout_result')=='undefined' || Cookie.get('orderout_result')==null)        
        Cookie.set('orderout_result',this.sendingQueryForm.result); else this.sendingQueryForm.result=Cookie.get('orderout_result');
    ...
    // -------------------------------------- *** ПРАВА *** ------------------------------------
   getSetOfPermissions(){
    return this.http.get('/api/auth/getMyPermissions?id=36')
            .subscribe(
                (data) => {   
  ...
  getCRUD_rights(permissionsSet:any[]){
    ...
    this.allowToDeleteAllCompanies = permissionsSet.some(         function(e){return(e==520)});
    this.allowToDeleteMyCompany = permissionsSet.some(            function(e){return(e==521)});
...
// -------------------------------------- *** КОНЕЦ ПРАВ *** ------------------------------------
  ...
  getTableHeaderTitles(){
    ...
    this.displayedColumns.push('expenditure');
  ...
  setNumOfPages(){
    ...
    this.createCheckedList();
    this.sendingQueryForm.offset=0;
    Cookie.set('orderout_result',this.sendingQueryForm.result);
    this.getData();
  }
  ...
  setPage(value:any) // set pagination
  {
    ...
    Cookie.set('orderout_offset',value);
    this.getData();
  }
  ...
  setSort(valueSortColumn:any) // set sorting column
  {
      ...
      Cookie.set('orderout_sortAsc',this.sendingQueryForm.sortAsc);
      } else {
          this.sendingQueryForm.sortColumn=valueSortColumn;
          this.sendingQueryForm.sortAsc="asc";
          Cookie.set('orderout_sortAsc',"asc");
          Cookie.set('orderout_sortColumn',valueSortColumn);
      }
      this.getData();
  }
  onCompanySelection(){
  ...
  undeleteDocs(){
    ...
    this.clearCheckboxSelection();
    return this.http.post('/api/auth/undeleteOrderout', body) 
    .subscribe(
      (data) => {  
  ...
  deleteDocs(){
    const body = {"checked": this.checkedList.join()}; //join переводит из массива в строку
    this.clearCheckboxSelection();
    return this.http.post('/api/auth/deleteOrderout', body) 
    .subscribe((data) => {         
    let result=data as any;
    ...
    openDialogSettings() { 
      ...
      dialogSettings.afterClosed().subscribe(result => {
        if(result){
          ...
          this.saveSettingsOrderout();
        }
      });
    }
    // Сохраняет настройки
    saveSettingsOrderout(){
