...
export class OrdersupComponent implements OnInit {
  ...
  constructor(private queryFormService:   QueryFormService,
    ...
    ngOnInit() {
      ...
      if(Cookie.get('ordersup_companyId')=='undefined' || Cookie.get('ordersup_companyId')==null)     
        Cookie.set('ordersup_companyId',this.sendingQueryForm.companyId); else this.sendingQueryForm.companyId=(Cookie.get('ordersup_companyId')=="0"?"0":+Cookie.get('ordersup_companyId'));
      if(Cookie.get('ordersup_departmentId')=='undefined' || Cookie.get('ordersup_departmentId')==null)  
        Cookie.set('ordersup_departmentId',this.sendingQueryForm.departmentId); else this.sendingQueryForm.departmentId=(Cookie.get('ordersup_departmentId')=="0"?"0":+Cookie.get('ordersup_departmentId'));
      if(Cookie.get('ordersup_sortAsc')=='undefined' || Cookie.get('ordersup_sortAsc')==null)       
        Cookie.set('ordersup_sortAsc',this.sendingQueryForm.sortAsc); else this.sendingQueryForm.sortAsc=Cookie.get('ordersup_sortAsc');
      if(Cookie.get('ordersup_sortColumn')=='undefined' || Cookie.get('ordersup_sortColumn')==null)    
        Cookie.set('ordersup_sortColumn',this.sendingQueryForm.sortColumn); else this.sendingQueryForm.sortColumn=Cookie.get('ordersup_sortColumn');
      if(Cookie.get('ordersup_offset')=='undefined' || Cookie.get('ordersup_offset')==null)        
...
// -------------------------------------- *** КОНЕЦ ПРАВ *** ------------------------------------
  ...
  setNumOfPages(){
    ...
    this.createCheckedList();
    this.sendingQueryForm.offset=0;
    Cookie.set('ordersup_result',this.sendingQueryForm.result);
    this.getData();
  }
  ...
  setPage(value:any) // set pagination
  {
    ...
    Cookie.set('ordersup_offset',value);
    this.getData();
  }
  ...
  setSort(valueSortColumn:any) // set sorting column
  {
      ...
      Cookie.set('ordersup_sortAsc',this.sendingQueryForm.sortAsc);
      } else {
          this.sendingQueryForm.sortColumn=valueSortColumn;
          this.sendingQueryForm.sortAsc="asc";
          Cookie.set('ordersup_sortAsc',"asc");
          Cookie.set('ordersup_sortColumn',valueSortColumn);
      }
      this.getData();
  }
  onCompanySelection(){
    Cookie.set('ordersup_companyId',this.sendingQueryForm.companyId);
    Cookie.set('ordersup_departmentId','0');
    // console.log('ordersup_companyId - '+Cookie.get('ordersup_companyId'));
    // console.log('ordersup_departmentId - '+Cookie.get('ordersup_departmentId'));
    this.sendingQueryForm.departmentId="0"; 
    this.resetOptions();
  ...
  setDefaultCompany(){
    if(Cookie.get('ordersup_companyId')=='0'||!this.companyIdInList(Cookie.get('ordersup_companyId'))){
