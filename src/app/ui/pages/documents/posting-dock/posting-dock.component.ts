...
    this.http.get('/api/auth/getPostingValuesById?id='+ this.id)
  ...
  checkDocNumberUnical() {
    if(!this.formBaseInformation.get('doc_number').errors)
    {
      ...
      return this.http.get('/api/auth/isDocumentNumberUnical?company_id='+this.formBaseInformation.get('company_id').value+'&doc_number='+this.formBaseInformation.get('doc_number').value+'&doc_id='+this.id+'&table=posting')
      .subscribe(
          (data) => {   
  ...
  goToNewDocument(){
    this._router.navigate(['ui/postingdock',0]);
    this.id=0;
...
//     });
//   }
//   formingProductRowLinkedDoc(row: PostingProductTable, dockname:string) {
//     return this._fb.group({
//       product_id: new FormControl (row.product_id,[]),
//       product_count: new FormControl (row.product_count,[]),
