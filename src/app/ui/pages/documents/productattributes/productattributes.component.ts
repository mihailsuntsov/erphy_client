...
import { QueryFormService } from './get-productattributes-table.service';
...
export class ProductAttributeComponent implements OnInit {
  ...
  receivedProductAttributeList: idAndName [] = [];//массив для получения списка расходов
  ...
  constructor(private queryFormService:   QueryFormService,
    ...
    public dialogRef1: MatDialogRef<ProductAttributeComponent>,
    ...
    ngOnInit() {
      ...
      if(Cookie.get('productattributes_companyId')=='undefined' || Cookie.get('productattributes_companyId')==null)     
        Cookie.set('productattributes_companyId',this.sendingQueryForm.companyId); else this.sendingQueryForm.companyId=(Cookie.get('productattributes_companyId')=="0"?"0":+Cookie.get('productattributes_companyId'));
      if(Cookie.get('productattributes_sortAsc')=='undefined' || Cookie.get('productattributes_sortAsc')==null)       
        Cookie.set('productattributes_sortAsc',this.sendingQueryForm.sortAsc); else this.sendingQueryForm.sortAsc=Cookie.get('productattributes_sortAsc');
      if(Cookie.get('productattributes_sortColumn')=='undefined' || Cookie.get('productattributes_sortColumn')==null)    
        Cookie.set('productattributes_sortColumn',this.sendingQueryForm.sortColumn); else this.sendingQueryForm.sortColumn=Cookie.get('productattributes_sortColumn');
      if(Cookie.get('productattributes_offset')=='undefined' || Cookie.get('productattributes_offset')==null)        
        Cookie.set('productattributes_offset',this.sendingQueryForm.offset); else this.sendingQueryForm.offset=Cookie.get('productattributes_offset');
      if(Cookie.get('productattributes_result')=='undefined' || Cookie.get('productattributes_result')==null)        
        Cookie.set('productattributes_result',this.sendingQueryForm.result); else this.sendingQueryForm.result=Cookie.get('productattributes_result');
  ...
  getCRUD_rights(){
    ...
    this.allowToDeleteAllCompanies = this.permissionsSet.some(         function(e){return(e==665)});
    this.allowToDeleteMyCompany = this.permissionsSet.some(            function(e){return(e==666)});
...
// -------------------------------------- *** КОНЕЦ ПРАВ *** ------------------------------------
  ...
  setNumOfPages(){
    ...
    this.createCheckedList();
    this.sendingQueryForm.offset=0;
    Cookie.set('productattributes_result',this.sendingQueryForm.result);
    this.getData();
  }
  ...
  setPage(value:any) // set pagination
  {
    ...
    Cookie.set('productattributes_offset',value);
    this.getData();
  }
  ...
  setSort(valueSortColumn:any) // set sorting column
  {
      ...
      Cookie.set('productattributes_sortAsc',this.sendingQueryForm.sortAsc);
      } else {
          this.sendingQueryForm.sortColumn=valueSortColumn;
          this.sendingQueryForm.sortAsc="asc";
          Cookie.set('productattributes_sortAsc',"asc");
          Cookie.set('productattributes_sortColumn',valueSortColumn);
      }
      this.getData();
  }
  onCompanySelection(){
    Cookie.set('productattributes_companyId',this.sendingQueryForm.companyId);
    this.resetOptions();
    this.getCRUD_rights();
  }
  ...
  getProductAttributeNameById(id:string):string{
    let name:string = translate('docs.msg.not_set');
    if(this.receivedProductAttributeList){
      this.receivedProductAttributeList.forEach(a=>{
  ...
  deleteDocs(){
    ...
    this.clearCheckboxSelection();
        return this.http.post('/api/auth/deleteProductAttribute', body) 
    .subscribe((data) => {   
  ...
  undeleteDocs(){
    const body = {"checked": this.checkedList.join()}; //join переводит из массива в строку
    this.clearCheckboxSelection();
     return this.http.post('/api/auth/undeleteProductAttribute', body) 
    .subscribe(
        (data) => {   
          let result=data as any;
          switch(result){
  ...
  setDefaultCompany(){
    if(Cookie.get('productattributes_companyId')=='0'||!this.companyIdInList(Cookie.get('productattributes_companyId'))){
  ...
  onClickRadioBtn(id:number, name:string){
    ...
    this.clearCheckboxSelection();
    return this.http.post('/api/auth/setDefaultCurrency', body) 
    .subscribe(
    (data) => {  
      let result=data as any;
      switch(result){
        case 1:{this.getData();this.openSnackBar(translate('menu.msg.currency_set',{name: name}), translate('menu.msg.close'));break;}  //+++
        case null:{this.MessageDialog.open(MessageDialog,{width:'400px',data:{head:translate('menu.msg.error'),message:(translate('menu.msg.error_msg'))}});break;}
