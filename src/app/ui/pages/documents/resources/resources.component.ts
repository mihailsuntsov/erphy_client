...
import { QueryFormService } from './get-resources-table.service';
...
export class ResourcesComponent implements OnInit {
  ...
  receivedResourcesList: idAndName [] = [];//массив для получения списка расходов
  ...
  constructor(private queryFormService:   QueryFormService,
    ...
    public dialogRef1: MatDialogRef<ResourcesComponent>,
    ...
    ngOnInit() {
      ...
      if(Cookie.get('resources_companyId')=='undefined' || Cookie.get('resources_companyId')==null)     
        Cookie.set('resources_companyId',this.sendingQueryForm.companyId); else this.sendingQueryForm.companyId=(Cookie.get('resources_companyId')=="0"?"0":+Cookie.get('resources_companyId'));
      if(Cookie.get('resources_sortAsc')=='undefined' || Cookie.get('resources_sortAsc')==null)       
        Cookie.set('resources_sortAsc',this.sendingQueryForm.sortAsc); else this.sendingQueryForm.sortAsc=Cookie.get('resources_sortAsc');
      if(Cookie.get('resources_sortColumn')=='undefined' || Cookie.get('resources_sortColumn')==null)    
        Cookie.set('resources_sortColumn',this.sendingQueryForm.sortColumn); else this.sendingQueryForm.sortColumn=Cookie.get('resources_sortColumn');
      if(Cookie.get('resources_offset')=='undefined' || Cookie.get('resources_offset')==null)        
        Cookie.set('resources_offset',this.sendingQueryForm.offset); else this.sendingQueryForm.offset=Cookie.get('resources_offset');
      if(Cookie.get('resources_result')=='undefined' || Cookie.get('resources_result')==null)        
        Cookie.set('resources_result',this.sendingQueryForm.result); else this.sendingQueryForm.result=Cookie.get('resources_result');
  ...
  getCRUD_rights(){
    ...
    this.allowToDeleteAllCompanies = this.permissionsSet.some(         function(e){return(e==686)});
    this.allowToDeleteMyCompany = this.permissionsSet.some(            function(e){return(e==687)});
...
// -------------------------------------- *** КОНЕЦ ПРАВ *** ------------------------------------
  ...
  setNumOfPages(){
    ...
    this.createCheckedList();
    this.sendingQueryForm.offset=0;
    Cookie.set('resources_result',this.sendingQueryForm.result);
    this.getData();
  }
  ...
  setPage(value:any) // set pagination
  {
    ...
    Cookie.set('resources_offset',value);
    this.getData();
  }
  ...
  setSort(valueSortColumn:any) // set sorting column
  {
      ...
      Cookie.set('resources_sortAsc',this.sendingQueryForm.sortAsc);
      } else {
          this.sendingQueryForm.sortColumn=valueSortColumn;
          this.sendingQueryForm.sortAsc="asc";
          Cookie.set('resources_sortAsc',"asc");
          Cookie.set('resources_sortColumn',valueSortColumn);
      }
      this.getData();
  }
  onCompanySelection(){
    Cookie.set('resources_companyId',this.sendingQueryForm.companyId);
    this.resetOptions();
    this.getCRUD_rights();
  }
  ...
  getResourcesNameById(id:string):string{
    let name:string = translate('docs.msg.not_set');
    if(this.receivedResourcesList){
      this.receivedResourcesList.forEach(a=>{
  ...
  undeleteDocs(){
    ...
    this.clearCheckboxSelection();
      return this.http.post('/api/auth/undeleteResource', body) 
    .subscribe(
        (data) => {   
  ...
  deleteDocs(){
    const body = {"checked": this.checkedList.join()}; //join переводит из массива в строку
    this.clearCheckboxSelection();
    return this.http.post('/api/auth/deleteResource', body) 
      .subscribe(
        (data) => {  
          let result=data as any;
          switch(result){ //+++
  ...
  setDefaultCompany(){
    if(Cookie.get('resources_companyId')=='0'||!this.companyIdInList(Cookie.get('resources_companyId'))){
