...
interface DockResponse {//интерфейс для получения ответа в методе getShipmentValuesById
  id: number;
  company: string;
  company_id: number;
  department: string;
...
@Component({
  selector: 'app-shipment-dock',
  templateUrl: './shipment-dock.component.html',
  styleUrls: ['./shipment-dock.component.css'],
...
export class ShipmentDockComponent implements OnInit {
...
//????????????????
    ...
    // if(this.productSearchAndTableComponent && this.productSearchAndTableComponent.getProductTable().length>0){
      ...
      this.http.post('/api/auth/insertShipment', this.formBaseInformation.value)
        ...
        (data) => {
                    ...
                    if (this.createdDockId==0){// 0 возвращает если не удалось сохранить изза превышения количества покупаемого товара над доступным количеством
                    ...
                    } else if (this.createdDockId==null){ //ошибка создания
                      ...
                      if (withReceipt){
                        ...
                        this.kkmComponent.printReceipt(21, this.createdDockId);//21 - Отгрузка);
  ...
  afterCreateShipment(withReceipt:boolean){// с true запрос придет при отбиваемом в данный момент чеке
    ...
    this._router.navigate(['/ui/shipmentdock', this.id]);
...
//устанавливает цвет статуса (используется для цветовой индикации статусов)
  ...
  goToNewDocument(){
      this._router.navigate(['ui/shipmentdock',0]);
      this.id=0;
