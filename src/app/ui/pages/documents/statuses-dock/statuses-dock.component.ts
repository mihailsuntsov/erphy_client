...
interface dockResponse {//интерфейс для получения ответа в методе getStatusDocksTableById
  id: number;
  company: string;
  company_id: number;
  document: string;
...
@Component({
  selector: 'app-statuses-dock',
  templateUrl: './statuses-dock.component.html',
  styleUrls: ['./statuses-dock.component.css'],
  providers: [LoadSpravService,]
})
export class StatusesDockComponent implements OnInit {
  ...
  ngOnInit() {
    this.formBaseInformation = new FormGroup({
      ...
      dock_id: new FormControl      (0,[Validators.required]),
...
// -------------------------------------- *** КОНЕЦ ПРАВ *** ------------------------------------
  ...
  setDefaultCompany(){
    ...
    {
      this.formBaseInformation.get('company_id').setValue(Cookie.get('satusdock_companyId')=="0"?this.myCompanyId:+Cookie.get('satusdock_companyId'));
    } else {
  ...
  setDefaultDocument(){
    ...
    if(this.receivedDocumentsList.length>1)
    {
      this.formBaseInformation.get('dock_id').setValue(Cookie.get('satusdock_documentId')=="0"?this.receivedDocumentsList[0].id:+Cookie.get('satusdock_documentId'))
    //} else {
      //this.formBaseInformation.get('dock_id').setValue(+this.receivedDocumentsList[0].id);
      //Cookie.set('satusdock_documentId',this.formBaseInformation.get('dock_id').value);
    }
  ...
          this.http.post('/api/auth/getSpravStatusDocksValuesById', dockId)
        .subscribe(
            data => { 
                ...
                this.formBaseInformation.get('dock_id').setValue(+documentValues.dock_id);
                this.formBaseInformation.get('dock').setValue(documentValues.dock);
  ...
  createNewDocument(){
    ...
    this.http.post('/api/auth/insertSpravStatusDocks', this.formBaseInformation.value)
            .subscribe(
                (data) =>   {
  ...
  updateDocument(complete:boolean){ 
    const body= {
      ...
      "dock_id":                  this.formBaseInformation.get('dock_id').value,
      ...
      return this.http.post('/api/auth/updateSpravStatusDocks', body)
        .subscribe(
            (data) => 
            {   
              this.getData();
  ...
  getStatusesList(){//возвращает список всех статусов документа, который нужен для изменения порядка вывода статусов
    const body = {  searchString: '', 
      ...
      documentId: this.formBaseInformation.get('dock_id').value};
      return this.http.post('/api/auth/getStatusDocksTable', body)
            .subscribe(
                (data) => {
