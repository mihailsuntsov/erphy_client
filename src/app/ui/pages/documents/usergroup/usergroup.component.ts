  ...
  constructor(private queryFormService:   QueryFormService,
    ...
    ngOnInit() {
      ...
      if(Cookie.get('usergroup_sortColumn')=='undefined' || Cookie.get('usergroup_sortColumn')==null)    
        Cookie.set('usergroup_sortColumn',this.sendingQueryForm.sortColumn); else this.sendingQueryForm.sortColumn=Cookie.get('usergroup_sortColumn');
      if(Cookie.get('usergroup_offset')=='undefined' || Cookie.get('usergroup_offset')==null)        
...
// -------------------------------------- *** КОНЕЦ ПРАВ *** ------------------------------------
  ...
  setNumOfPages(){
    ...
    this.createCheckedList();
    this.sendingQueryForm.offset=0;
    Cookie.set('usergroup_result',this.sendingQueryForm.result);
    this.getData();
  }
  ...
  setPage(value:any) // set pagination
  {
    ...
    Cookie.set('usergroup_offset',value);
    this.getData();
  }
  ...
  setSort(valueSortColumn:any) // set sorting column
  {
      ...
      Cookie.set('usergroup_sortAsc',this.sendingQueryForm.sortAsc);
      } else {
          this.sendingQueryForm.sortColumn=valueSortColumn;
          this.sendingQueryForm.sortAsc="asc";
          Cookie.set('usergroup_sortAsc',"asc");
          Cookie.set('usergroup_sortColumn',valueSortColumn);
      }
      this.getData();
  }
  onCompanySelection(){
