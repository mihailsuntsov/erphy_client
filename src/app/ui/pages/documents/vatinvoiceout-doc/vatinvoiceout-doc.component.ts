...
interface DocResponse {//интерфейс для получения ответа в методе getVatinvoiceoutValuesById
  id: number;
  company: string;
  company_id: string;
  creator: string;
...
@Component({
  selector: 'app-vatinvoiceout-doc',
  templateUrl: './vatinvoiceout-doc.component.html',
  styleUrls: ['./vatinvoiceout-doc.component.css'],
  providers: [LoadSpravService, CommonUtilitesService,BalanceCagentComponent,
    { provide: DateAdapter, useClass: MomentDateAdapter,deps: [MAT_DATE_LOCALE, MAT_MOMENT_DATE_ADAPTER_OPTIONS]}, //+++
    {provide: MAT_DATE_FORMATS, useValue: MY_FORMATS},
  ]
})
export class VatinvoiceoutDocComponent implements OnInit {
  ...
  constructor(private activateRoute: ActivatedRoute,
    ...
    private _fb: UntypedFormBuilder, //чтобы билдить группу форм vatinvoiceoutProductTable    
    public SettingsVatinvoiceoutDialogComponent: MatDialog,
  ...
  ngOnInit(): void {
    this.formBaseInformation = new UntypedFormGroup({
      ...
      cagent2_id: new UntypedFormControl               ('',[]),
    ...
    this.formLinkedDocs = new UntypedFormGroup({
      ...
      vatinvoiceout_id: new UntypedFormControl   ('',[]), //для счёта фактуры выданного
  ...
  getSetOfPermissions(){
    return this.http.get('/api/auth/getMyPermissions?id=37')
      .subscribe(
          (data) => {   
  ...
  getCagents2List(){ //заполнение Autocomplete
    try {
      if(this.canCagent2AutocompleteQuery && this.searchCagent2Ctrl.value.length>1){
        const body = {
          "searchString":this.searchCagent2Ctrl.value,
          "companyId":this.formBaseInformation.get('company_id').value};
        this.isCagent2ListLoading  = true;
        return this.http.post('/api/auth/getCagentsList', body);
      }else return [];
    } catch (e) {
      return [];}}

  onCagent2SearchValueChanges(){
    this.searchCagent2Ctrl.valueChanges
    .pipe(
      debounceTime(500),
      tap(() => {
        this.filteredCagents2 = [];}),       
      switchMap(fieldObject =>  
        this.getCagents2List()))
    .subscribe(data => {
      this.isCagent2ListLoading = false;
      if (data == undefined) {
        this.filteredCagents2 = [];
      } else {
        this.filteredCagents2 = data as any;
  }});}

  onSelectCagent2(id:any,name:string){
    this.formBaseInformation.get('cagent2_id').setValue(+id);
  }
    
  checkEmptyCagent2Field(){
    if(this.searchCagent2Ctrl.value.length==0){
      this.formBaseInformation.get('cagent2_id').setValue(null);
  }}
//-------------------------------------------------------------------------------
...
getCompaniesList(){ //+++
  ...
  getCRUD_rights(){
    this.allowToCreateAllCompanies = this.permissionsSet.some(         function(e){return(e==487)});
    this.allowToCreateMyCompany = this.permissionsSet.some(            function(e){return(e==488)});
    this.allowToViewAllCompanies = this.permissionsSet.some(           function(e){return(e==491)});
    this.allowToViewMyCompany = this.permissionsSet.some(              function(e){return(e==492)});
    this.allowToUpdateAllCompanies = this.permissionsSet.some(         function(e){return(e==493)});
    this.allowToUpdateMyCompany = this.permissionsSet.some(            function(e){return(e==494)});
    this.allowToCompleteAllCompanies = this.permissionsSet.some(       function(e){return(e==495)});
    this.allowToCompleteMyCompany = this.permissionsSet.some(          function(e){return(e==496)});
  ...
  getStatusesList(){
    ...
    this.loadSpravService.getStatusList(this.formBaseInformation.get('company_id').value,37) //37 - id документа из таблицы documents
      .subscribe(
  ...
  getDocumentValuesById(){
    this.http.get('/api/auth/getVatinvoiceoutValuesById?id='+ this.id)
        .subscribe(
            data => { 
  ...
  setDocumentAsDecompleted(){
    this.http.post('/api/auth/setVatinvoiceoutAsDecompleted',  this.formBaseInformation.value)
      .subscribe(
          (data) => 
          {   
            ...
            switch(result){
              ...
              case 1:{// Успешно
                this.openSnackBar(translate('docs.msg.cnc_com_succs',{name:translate('docs.docs.v_invoiceout')}), translate('docs.msg.close'));
                // this.getLinkedDocsScheme(true);//загрузка диаграммы связанных документов
  ...
  updateDocument(complete?:boolean){ 
    ...
    this.http.post('/api/auth/updateVatinvoiceout',  this.formBaseInformation.value)
      .subscribe(
          (data) => 
          {   
            if(complete){
            ...
            switch(result){
              case null:{// null возвращает если не удалось создать документ из-за ошибки
                this.MessageDialog.open(MessageDialog,{width:'400px',data:{head:translate('docs.msg.error'),message:translate('docs.msg.error_of') + (complete?translate('docs.msg._of_comp'):translate('docs.msg._of_save')) + translate('docs.msg._of_doc',{name:translate('docs.docs.v_invoiceout')})}});
                break;
              }
              case -1:{//недостаточно прав
                this.MessageDialog.open(MessageDialog,{width:'400px',data:{head:translate('docs.msg.error'),message:translate('docs.msg.ne_perm_oper') + (complete?translate('docs.msg._of_comp'):translate('docs.msg._of_save')) + translate('docs.msg._of_doc',{name:translate('docs.docs.v_invoiceout')})}});
                break;
              }
              ...
              default:{// Успешно
                this.openSnackBar(translate('docs.msg.doc_name',{name:translate('docs.docs.v_invoiceout')}) + (complete?translate('docs.msg.completed'):translate('docs.msg.saved')), translate('docs.msg.close'));
                if(complete) {
  ...
  getSettings(){
    let result:any;
    this.http.get('/api/auth/getSettingsVatinvoiceout')
      .subscribe(
          data => { 
            result=data as any;
            ...
            if(this.isCompanyInList(+result.companyId)){
              this.settingsForm.get('cagent2Id').setValue(result.cagent2Id);
              this.settingsForm.get('cagent2').setValue(result.cagent2);
  ...
  setDefaultInfo(){
      if(+this.settingsForm.get('cagent2Id').value>0 && +this.formBaseInformation.get('cagent2_id').value==0 && !this.formBaseInformation.get('is_completed').value){
        this.searchCagent2Ctrl.setValue(this.settingsForm.get('cagent2').value);
        this.formBaseInformation.get('cagent2_id').setValue(this.settingsForm.get('cagent2Id').value);
        this.formBaseInformation.get('cagent2').setValue(this.settingsForm.get('cagent2').value);
      }
  }
  ...
  openDialogSettings() { 
    ...
    dialogSettings.afterClosed().subscribe(result => {
      if(result){
        ...
        if(result.get('cagent2Id')) this.settingsForm.get('cagent2Id').setValue(result.get('cagent2Id').value);
        if(result.get('cagent2')) this.settingsForm.get('cagent2').setValue(result.get('cagent2').value);
        this.settingsForm.get('statusIdOnComplete').setValue(result.get('statusIdOnComplete').value);
        this.saveSettingsVatinvoiceout();
      }
    });
  }

  saveSettingsVatinvoiceout(){
    return this.http.post('/api/auth/saveSettingsVatinvoiceout', this.settingsForm.value)
            .subscribe(
                (data) => {   
...
openDialogAddFiles() {
  ...
  dialogRef.afterClosed().subscribe(result => {
    ...
    if(result)this.addFilesToVatinvoiceout(result);
  });
}
...
loadFilesInfo(){//                                     загружает информацию по прикрепленным файлам
    return this.http.get('/api/auth/getListOfVatinvoiceoutFiles?id='+this.id) 
          .subscribe(
              (data) => {  
...
addFilesToVatinvoiceout(filesIds: number[]){
  const body = {"id1":this.id, "setOfLongs1":filesIds};// передаем id товара и id файлов 
          return this.http.post('/api/auth/addFilesToVatinvoiceout', body) 
            .subscribe(
                (data) => {  
                  this.loadFilesInfo();
...
deleteFile(id:number){
  ...
  return this.http.post('/api/auth/deleteVatinvoiceoutFile',body)
  .subscribe(
      (data) => {   
                  this.loadFilesInfo();
...
//**********************************************************************************************************************************************/  
  ...
  createLinkedDoc(docname:string){// принимает аргументы: Return
    ...
    if(canCreateLinkedDoc.can){
      ...
      this.formLinkedDocs.get('description').setValue(translate('docs.msg.created_from')+translate('docs.docs.v_invoiceout')+' '+translate('docs.top.number')+this.formBaseInformation.get('doc_number').value);
      ...
      this.formLinkedDocs.get('linked_doc_name').setValue('vatinvoiceout');//имя (таблицы) связанного документа

      //поля для счёта-фактуры выданного
      this.formLinkedDocs.get('parent_tablename').setValue('vatinvoiceout');
      this.formLinkedDocs.get('vatinvoiceout_id').setValue(this.id);
  ...
  openDialogTemplates() { 
    const dialogTemplates = this.templatesDialogComponent.open(TemplatesDialogComponent, {
      ...
      { //отправляем в диалог:
        ...
        document_id: 37, // id документа из таблицы documents
      },
    });
  ...
  printDocs(){
    ...
    this.templatesList=[];
    this.http.get('/api/auth/getTemplatesList?company_id='+this.formBaseInformation.get('company_id').value+"&document_id="+37+"&is_show="+true).subscribe
    (data =>{ 
  ...
  clickOnTemplate(template:TemplatesList){
    const baseUrl = '/api/auth/vatinvoiceoutPrint/';
...
//*****************************************************************************************************************************************/
  ...
  getFormIngexByProductId(productId:number):number{
    ...
    this.formBaseInformation.value.vatinvoiceoutProductTable.map(i => 
      {
