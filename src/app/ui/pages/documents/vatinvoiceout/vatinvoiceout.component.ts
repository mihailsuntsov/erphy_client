...
@Component({
  selector: 'app-vatinvoiceout',
  templateUrl: './vatinvoiceout.component.html',
  styleUrls: ['./vatinvoiceout.component.css'],
  providers: [QueryFormService,LoadSpravService,Cookie,CommonUtilitesService] //+++
})
export class VatinvoiceoutComponent implements OnInit {
 ...
 visBtnDelete = false;
  ...
  constructor(private queryFormService:   QueryFormService,
    ...
    private settingsVatinvoiceoutDialogComponent: MatDialog,
    public deleteDialog: MatDialog,
    public dialogRef1: MatDialogRef<VatinvoiceoutComponent>,
    ...
    ngOnInit() {
      ...
      if(Cookie.get('vatinvoiceout_companyId')=='undefined' || Cookie.get('vatinvoiceout_companyId')==null)     
        Cookie.set('vatinvoiceout_companyId',this.sendingQueryForm.companyId); else this.sendingQueryForm.companyId=(Cookie.get('vatinvoiceout_companyId')=="0"?"0":+Cookie.get('vatinvoiceout_companyId'));
      if(Cookie.get('vatinvoiceout_sortAsc')=='undefined' || Cookie.get('vatinvoiceout_sortAsc')==null)       
        Cookie.set('vatinvoiceout_sortAsc',this.sendingQueryForm.sortAsc); else this.sendingQueryForm.sortAsc=Cookie.get('vatinvoiceout_sortAsc');
      if(Cookie.get('vatinvoiceout_sortColumn')=='undefined' || Cookie.get('vatinvoiceout_sortColumn')==null)    
        Cookie.set('vatinvoiceout_sortColumn',this.sendingQueryForm.sortColumn); else this.sendingQueryForm.sortColumn=Cookie.get('vatinvoiceout_sortColumn');
      if(Cookie.get('vatinvoiceout_offset')=='undefined' || Cookie.get('vatinvoiceout_offset')==null)        
        Cookie.set('vatinvoiceout_offset',this.sendingQueryForm.offset); else this.sendingQueryForm.offset=Cookie.get('vatinvoiceout_offset');
      if(Cookie.get('vatinvoiceout_result')=='undefined' || Cookie.get('vatinvoiceout_result')==null)        
        Cookie.set('vatinvoiceout_result',this.sendingQueryForm.result); else this.sendingQueryForm.result=Cookie.get('vatinvoiceout_result');
  ...
  getCRUD_rights(permissionsSet:any[]){
    ...
    this.allowToDeleteAllCompanies = permissionsSet.some(         function(e){return(e==489)});
    this.allowToDeleteMyCompany = permissionsSet.some(            function(e){return(e==490)});
...
// -------------------------------------- *** КОНЕЦ ПРАВ *** ------------------------------------
  ...
  setNumOfPages(){
    ...
    this.createCheckedList();
    this.sendingQueryForm.offset=0;
    Cookie.set('vatinvoiceout_result',this.sendingQueryForm.result);
    this.getData();
  }
  ...
  setPage(value:any) // set pagination
  {
    ...
    Cookie.set('vatinvoiceout_offset',value);
    this.getData();
  }
  ...
  setSort(valueSortColumn:any) // set sorting column
  {
      ...
      Cookie.set('vatinvoiceout_sortAsc',this.sendingQueryForm.sortAsc);
      } else {
          this.sendingQueryForm.sortColumn=valueSortColumn;
          this.sendingQueryForm.sortAsc="asc";
          Cookie.set('vatinvoiceout_sortAsc',"asc");
          Cookie.set('vatinvoiceout_sortColumn',valueSortColumn);
      }
      this.getData();
  }
  onCompanySelection(){
    Cookie.set('vatinvoiceout_companyId',this.sendingQueryForm.companyId);
    this.resetOptions();
    this.getData();
  }
  clickBtnDelete(): void {
  ...
  undeleteDocs(){
    ...
    this.clearCheckboxSelection();
    return this.http.post('/api/auth/undeleteVatinvoiceout', body) 
    .subscribe(
      (data) => {   
  ...
  deleteDocs(){
    const body = {"checked": this.checkedList.join()}; //join переводит из массива в строку
    this.clearCheckboxSelection();
    return this.http.post('/api/auth/deleteVatinvoiceout', body) 
    .subscribe((data) => {    
      let result=data as any;
      switch(result.result){
        ...
        case 3:{let numbers:string='';
          for(var i=0;i<result.docs.length;i++){numbers=numbers+' <a href="ui/vatinvoiceoutdoc/'+result.docs[i].id+'">'+result.docs[i].doc_number+'</a>';}
  ...
  setDefaultCompany(){
    if(Cookie.get('vatinvoiceout_companyId')=='0'||!this.companyIdInList(Cookie.get('vatinvoiceout_companyId'))){
  ...
  onClickCreateBtn(){
    this.MessageDialog.open(MessageDialog,{width:'400px',data:{head:translate('menu.msg.attention'),message:translate('menu.msg.vii_can_creat')}})
  }
  //*************************************************************   НАСТРОЙКИ   ************************************************************/    
