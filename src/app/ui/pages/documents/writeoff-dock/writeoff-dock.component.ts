...
export class WriteoffDockComponent implements OnInit {
  ...
  constructor(private activateRoute: ActivatedRoute,
    ...
    private _fb: FormBuilder, //чтобы билдить группу форм writeoffProductTable
...
    this.http.get('/api/auth/getWriteoffValuesById?id='+ this.id)
  ...
  formingProductRowFromApiResponse(row: WriteoffProductTable) {
    return this._fb.group({
      ...
      writeoff_id:        new FormControl (this.id,[]),
  ...
  checkDocNumberUnical() {
    if(!this.formBaseInformation.get('doc_number').errors)
    {
      ...
      return this.http.get('/api/auth/isDocumentNumberUnical?company_id='+this.formBaseInformation.get('company_id').value+'&doc_number='+this.formBaseInformation.get('doc_number').value+'&doc_id='+this.id+'&table=writeoff')
      .subscribe(
          (data) => {   
          ...
          {   
            ...
            switch(result){
              ...
              case 0:{//недостаточно товара на складе
                this.MessageDialog.open(MessageDialog,{width:'400px',data:{head:'Ошибка!',message:"Количество тоара на складе у одной или нескольких позиций несдостаточно для списания"}});
                break;
              }
  ...
  goToNewDocument(){
    this._router.navigate(['ui/writeoffdock',0]);
    this.id=0;
...
//       this.formReturnsup.get('description').setValue('Создано из списания №'+ this.formBaseInformation.get('doc_number').value);
...
//     });
//   }
//   formingProductRowLinkedDoc(row: WriteoffProductTable, dockname:string) {
//     return this._fb.group({
//       product_id: new FormControl (row.product_id,[]),
//       product_count: new FormControl (row.product_count,[]),
