...
export class WriteoffComponent implements OnInit {
  ...
  constructor(private queryFormService:   QueryFormService,
    ...
    private settingsWriteoffDialogComponent: MatDialog,
  ...
  getCRUD_rights(permissionsSet:any[]){
    ...
    this.allowToDeleteAllCompanies = permissionsSet.some(         function(e){return(e==219)});
    this.allowToDeleteMyCompany = permissionsSet.some(            function(e){return(e==220)});
    this.allowToDeleteMyDepartments = permissionsSet.some(        function(e){return(e==221)});
    this.allowToDeleteMyDocs = permissionsSet.some(               function(e){return(e==222)});
...
// -------------------------------------- *** КОНЕЦ ПРАВ *** ------------------------------------
  ...
  getTableHeaderTitles(){
    ...
    this.displayedColumns.push('writeoff_date');
  ...
  setNumOfPages(){
    ...
    this.createCheckedList();
    this.sendingQueryForm.offset=0;
    Cookie.set('writeoff_result',this.sendingQueryForm.result);
    this.getData();
  }
  ...
  setPage(value:any) // set pagination
  {
    ...
    Cookie.set('writeoff_offset',value);
    this.getData();
  }
  ...
  setSort(valueSortColumn:any) // set sorting column
  {
      ...
      Cookie.set('writeoff_sortAsc',this.sendingQueryForm.sortAsc);
      } else {
  ...
  onCompanySelection(){
    Cookie.set('writeoff_companyId',this.sendingQueryForm.companyId);
    Cookie.set('writeoff_departmentId','0');
    // console.log('writeoff_companyId - '+Cookie.get('writeoff_companyId'));
    // console.log('writeoff_departmentId - '+Cookie.get('writeoff_departmentId'));
    this.sendingQueryForm.departmentId="0"; 
    this.resetOptions();
    this.getDepartmentsList();
  }
  onDepartmentSelection(){
    Cookie.set('writeoff_departmentId',this.sendingQueryForm.departmentId);
    // console.log('writeoff_companyId - '+Cookie.get('writeoff_companyId'));
    // console.log('writeoff_departmentId - '+Cookie.get('writeoff_departmentId'));
    this.resetOptions();
    this.getData();
  ...
  .subscribe((data) => {   
    ...
    switch(result.result){
      ...
      case 3:{let numbers:string='';
        for(var i=0;i<result.docs.length;i++){numbers=numbers+' <a href="/ui/writeoffdock/'+result.docs[i].id+'">'+result.docs[i].doc_number+'</a>';}
  ...
  setDefaultCompany(){
    if(Cookie.get('writeoff_companyId')=='0'){
  ...
  clickBtnRestore(): void {
    const dialogRef = this.confirmDialog.open(ConfirmDialog, {
      ...
      { 
        head: 'Восстановление',
        query: 'Восстановить выбранные списания из удалённых?',
        warning: '',
      },
    });
    dialogRef.afterClosed().subscribe(result => {
  ...
  undeleteDocks(){
    const body = {"checked": this.checkedList.join()}; //join переводит из массива в строку
    this.clearCheckboxSelection();
      return this.http.post('/api/auth/undeleteWriteoff', body) 
    .subscribe(
        (data) => {   
                    this.getData();
